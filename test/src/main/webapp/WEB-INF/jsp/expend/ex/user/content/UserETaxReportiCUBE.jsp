...
<script>
	...
	function fnSetEtaxData(data){
		...
		$.each(data,function(idx, val){
			...
			tEtaxData.divNm = val[0].divNm; // 사업장명
	...
	function fnSetGrid(data){
		...
		if(pageLength == 0){
			...
			$(tr).append('<td colspan="' + $("#tblUserETaxReport colgroup col").length + '">데이터가 없습니다.</td>')
	...
	<div class="btn_div cl">
		...
		<div class="right_div">							
			<div class="controll_btn p0">
				...
				<button id="btnExcel">${CL.ex_excelDownload} <!--엑셀다운로드--></button>
			</div>
		</div>
	</div>
