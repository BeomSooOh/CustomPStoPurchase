...
<script>
	...
	function fnUserETaxListSearch() {
		...
		// param.sendType = ($('#selSendType').val() || '');
		...
		$.each(data,function(idx, val){
			...
			switch( $("#selSendType").val() ){
				case "2" : /* 결의 */
					...
					break;
				case "1" : /* 미결의 */
					if(val.length == 1 || ( val.length > 1 && val[1].sendYN == 'N' && val[1].ifMId != 'D') ){
	...
	function fnSetGrid(data){
		...
		if(pageLength == 0){
			...
			$(tr).append('<td colspan="' + $("#tblUserETaxReport colgroup col").length + '">데이터가 없습니다. iU [결의서입력(자동전표입력)]메뉴에서 [전자세금계산서처리]를 확인하시기 바랍니다.</td>')
			...
			$.each(data,function(idx, val){
				if( (( currentPage - 1) * $("#selViewLength").val()) < val.rowNum && val.rowNum <= (currentPage * $("#selViewLength").val())){
					...
					$(tr).append('<td class="ce">' + '<a class="text_blue etaxPop" style="text-decoration:underline;cursor:pointer;" title="매입전자 세금 계산서 상세 팝업보기">' + val.hometaxIssNo + '</a>' + '</td>'); //승인번호에서 국세청승인번호로 교체 by Kwon Oh Gwang on 2019-04-18.
	...
	function fnAdminReportExcelDown() {
		...
		/* Excel 헤더 정의 */
		/*정성일 210908 - 리스트 엑셀다운로드시 국세청승인번호표시 (iU전용)*/
		var excelHeader = {};
		excelHeader.issDt = '${CL.ex_writeDate}';
		excelHeader.trNm =  '${CL.ex_supplier}';
		excelHeader.trregNb =  '${CL.ex_corporateRegistrationNumber}';
		excelHeader.issNo =  '${CL.ex_confirmationNumber}';
		excelHeader.hometaxIssNo =  '국세청 번호';
		...
		excelHeader.sendType =  '${CL.ex_resCondition}';
		excelHeader.empName =  '${CL.ex_resPerson}';
	...
	function fnAdminSetETaxUseYN(ynType){
		...
		for(var i = 0 ; i <chkSels.length ; i++){
			...
			/* param.sendYN = ynType;  해당 컬럼은 상신 상태값컬럼으로 수정 되어서 안됨 : 상신 상태일때 이관취소불가함 */
			/* 기존의 미사용건들에 대해,  send_yn 값이 Y로 지정되어 있기에, 미사용 및 미사용시 send_yn = 'N'값으로 하드코딩 */
			param.sendYN = 'N';
	...
	function fnUserTransfer(){
		// if(!confirm("이관이 완료되면 기존 조회 권한자는 해당정보를 조회할 수 없습니다.\n계속 진행하시겠습니까?")){
        // 	return false;
        // }
