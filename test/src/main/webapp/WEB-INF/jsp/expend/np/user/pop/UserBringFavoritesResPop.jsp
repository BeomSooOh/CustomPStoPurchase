...
<script>
	...
	function fnFavoritesListSelect(){
		...
		$.ajax({
	        type : 'post',
	        url : '<c:url value="/ex/np/user/res/ResFavoritesListSelect.do" />',
	        datatype : 'json',
	        async : true,
	        data : data,
	...
	function fnDrawFavoritesInfo(data){
		...
		for(var i=0; i< length; i++){
			...
			trCode += ' <td style="text-align:left;padding-left:10px;"><a class="favoriteDoc" onclick="fnFavoritesUpdate(\'' + escape(JSON.stringify(item)) + '\')" style="cursor:pointer"><img src="/exp/Images/ico/ico_book01_on.png" index="' + i + '" id="' + item.resDocSeq + '"></a>' + ' ' + item.resDocNote + '</td>';
	...
	function fnSearchListFilter(aaData){
		...
		for(var i=0; i<aaData.length;i++){
			...
			if(aaData[i].docNo.indexOf(searchCode)==-1 && aaData[i].resDocNote.indexOf(searchCode)==-1){
				continue;
			}		
	...
	function fnFavoritesUpdate(aData){
		...
		var resDocSeq = aData.resDocSeq;
		var param = {
				    resDocSeq : aData.resDocSeq
				    , favoritesStatus : 'N'
				    , resDocNote : aData.resDocNote
					};
		$.ajax({
		    type : 'post'
            ...
            <table border="0">
                ...
                <thead>
                    <tr>
                        ...
                        <th>${CL.ex_originDocTitle}  <!--제목--></th>
                        <th>${CL.ex_originDocDate}  <!--저장일자--></th>
