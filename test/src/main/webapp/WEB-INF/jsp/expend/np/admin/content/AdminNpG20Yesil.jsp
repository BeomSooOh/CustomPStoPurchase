...
<div class="sub_contents_wrap">
	...
	<div class="top_box">
		<dl>
			...
			<dd>
				<input type="text" autocomplete="off" style="width: 170px;" value="" id="txtGisuFromDate" class="puddSetup" pudd-type=""  disabled="true"/> 
					~ 
				<input type="text" autocomplete="off" style="width: 170px;" id="txtGisuToDate" value="" class="puddSetup" pudd-type=""  disabled="true"/>
			</dd>
		...
		<dl class="next2">
			...
			<dt class="ar divBottomArea" style="width:70px; display:none;" id="txt_mgtType">${CL.ex_bottomBussines}  <!--하위사업--></dt>
			<dd class="divBottomArea" style=" display:none;">
				<input type="text" autocomplete="off" style="width:306px;" class="puddSetup" id="txtErpBottomName" readonly/>
		...
		<dl class="next2">
			...
			<dd>	
				<select id="selOpt13List" class="puddSetup" pudd-style="width:359px;" />
	...
	<div class="btn_div cl">
		<div class="left_div">			
			...
			<div class="fl ml10 mt3" id="checkErpOptionSet">
				<input type="checkbox" name="inp_chk" id="checkBgt10" class="checkBgt"/><label class="" for="checkBgt10" id="checkBgt10-1">1레벨&nbsp;<!--10--></label>
				<input type="checkbox" name="inp_chk" id="checkBgt9" class="checkBgt"/><label class="" for="checkBgt9" id="checkBgt9-1">2레벨&nbsp;<!--9--></label>
				<input type="checkbox" name="inp_chk" id="checkBgt8" class="checkBgt"/><label class="" for="checkBgt8" id="checkBgt8-1">3레벨&nbsp;<!--8--></label>
				<input type="checkbox" name="inp_chk" id="checkBgt7" class="checkBgt"/><label class="" for="checkBgt7" id="checkBgt7-1">4레벨&nbsp;<!--7--></label>
				<input type="checkbox" name="inp_chk" id="checkBgt6" class="checkBgt"/><label class="" for="checkBgt6" id="checkBgt6-1">5레벨&nbsp;<!--6--></label>
				<input type="checkbox" name="inp_chk" id="checkBgt5" class="checkBgt"/><label class="" for="checkBgt5" id="checkBgt5-1">6레벨&nbsp;<!--5--></label>
				<input type="checkbox" name="inp_chk" id="checkBgt1" class="checkBgt"/><label class="" for="checkBgt1">${CL.ex_bgt1Name}&nbsp;<!--관--></label>
...
<script>
	...
	function fnAdminReportExcelDown() {
		...
		if(erpOptionSet[9].USE_YN=='1'){
			excelHeader.erpAllocationAmt = "기간배정액";	
		}
	...
	**/
	...
	function fnGetCommonParam(){
		...
		if(erpOptionSet[21].USE_YN!='0'){
			if( $('#checkBgt1').prop('checked') ){
				...
				divCode = 10;
				divCodeStandard = 10;
			} else if( $('#checkBgt5').prop('checked') ){
				divCode = 6;
				divCodeStandard = 6;
			} else if( $('#checkBgt6').prop('checked') ){
				divCode = 5;
				divCodeStandard = 5;
			} else if( $('#checkBgt7').prop('checked') ){
				divCode = 4;
				divCodeStandard = 4;
			} else if( $('#checkBgt8').prop('checked') ){
				divCode = 3;
				divCodeStandard = 3;
			} else if( $('#checkBgt9').prop('checked') ){
				divCode = 2;
				divCodeStandard = 2;
			} else if( $('#checkBgt10').prop('checked') ){
				divCode = 1;
				divCodeStandard = 1;
			}
		} else {
	...
	function fnSetYesilTable(param){
		...
		for(var i=0; i < aaData.length; i++){
			...
			if( item.erpBudgetLevelStandard > divCodeStandard ) {
				continue;
			}
			...
			success : function(result) {
				if(result.result.resultCode == 'SUCCESS'){
					...
					Pudd( "#selDivList" ).puddComboBox({
					    attributes : { style : "width:360px;" }// control 부모 객체 속성 설정
	...
	**/
	...
	function fnOpenAdminYesilERPResAmtDetailInfo(param){
		...
		$('#TOTAL_CRITERIA').val( parseInt($('#selOpt13List').val()) - 1 );
	...
	function fnOpenAdminYesilResAmtDetailInfo(param){
		...
		$('#ERP_DIV_CD').val( Pudd( "#selDivList" ).getPuddObject().val());
		$('#ERP_DIV_NAME').val(Pudd( "#selDivList" ).getPuddObject().text());
		$('#OPTION_TYPE').val($("#selOpt12List").val());
...
<form id="yesilBudgetInfoPop" name="frmPop" method="post">
	...
	<input type="hidden" name="erpTotalCriteria" value="" id="TOTAL_CRITERIA"/>
