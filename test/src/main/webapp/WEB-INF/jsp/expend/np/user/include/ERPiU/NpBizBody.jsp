...
<div class="pop_con">
	...
	<div class="top_box">
		<dl>
			<dt>${CL.ex_accountingUnitEndDate}  <!--회계단위종료일--></dt>
			<dd>
				<div class="dal_div">
		...
		if(data.length == 0){
			...
			$.each(data,function(idx, val){
				if ( !( ( searchType == '' ? JSON.stringify ( val ) : ( val [ searchType ] ? val [ searchType ] : '' ) ).indexOf ( $ ( '#txtSearchStr' ).val ( ) ) < 0 )){
					...
					var tr = document.createElement('tr');
					$(tr).data('data', val);
					$(tr).append('<td>' + val.ERPBIZSEQ + '</td>');
					$(tr).append('<td>' + val.ERPBIZNAME + '</td>');
					$(tr).append('<td>' + (!!val.REGNB?val.REGNB.substr(0,3) + '-' + val.REGNB.substr(3,2) + '-' + val.REGNB.substr(5,5):'') + '</td>');
					$("#tblCommonCode").append(tr);
