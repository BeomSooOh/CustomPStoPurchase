...
<div class="pop_con">
	...
	<div class="com_ta2 sc_head">
		<table>
			...
			<tr>
				<th><input type="checkbox" id="chk_budgetAllChk"/></th>
				<th>${CL.ex_code}  <!--코드--></th>
...
// 		$("#title").text("예산단위");
		...
		$('#chk_budgetAllChk').click(function (){
			$('.chkBudgetCode').prop('checked', $(this).prop('checked'));
		});
	}
		...
		if(data.length == 0){
			...
			$.each(data,function(idx, val){
				if ( !( ( searchType == '' ? JSON.stringify ( val ) : ( val [ searchType ] ? val [ searchType ] : '' ) ).indexOf ( $ ( '#txtSearchStr' ).val ( ) ) < 0 )){
					...
					var tr = document.createElement('tr');
					$(tr).data('data', val);
					$(tr).append('<td> <input type="checkbox" class="chkBudgetCode" value="' + escape(JSON.stringify(val)) + '"/> </td>')
