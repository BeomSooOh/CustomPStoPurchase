...
<!-- NpErphincomeiclistBody -->
<!-- 테이블 -->
<div class="pop_head">
	<h1>${CL.ex_businessIncomeEarner}</h1>
...
<div class="pop_con">
		...
		<dl class="">
			...
			<dd>
				<select class="puddSetup" pudd-style="width:100px" id="selSearchType">
					...
					<option value="ACCT_NM">${CL.ex_incomeearner}</option>
					<option value="REG_NO">${CL.ex_peopleNum}</option>
				</select>
			</dd>
			<dd>
	...
	<div class="com_ta2 sc_head">
		<table>
			...
			<tr>
				<th>${CL.ex_code}</th>
				<th>${CL.ex_incomeearner}</th>
				<th>${CL.ex_peopleNum}</th>
			</tr>
		</table>
	</div>
	...
	function fnGridCommonCodeTable ( data ) {
		...
		$.each ( data, function ( idx, val ) {
			if ( !( ( searchType == '' ? JSON.stringify ( val ) : ( val [ searchType ] ? val [ searchType ] : '' ) ).indexOf ( $ ( '#txtSearchStr' ).val ( ) ) < 0 )) {
				...
				if((val.REG_NO || '') !== '' && (val.REG_NO || '').replace(/-/g, '').toString().length >= 7){
					var regNo = [(val.REG_NO || '').replace(/-/g, '').substring(0, 6), ((val.REG_NO || '').replace(/-/g, '').substring(6, 7) + '******')].join('-');
					$ ( tr ).append ( '<td>' + regNo + '</td>' )
				} else {
					$ ( tr ).append ( '<td>' + val.REG_NO + '</td>' )
				}
