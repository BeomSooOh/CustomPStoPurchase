...
<div class="iframe_wrap" style="min-width:1000px;">
	...
	<div class="SearchDetail" id="divSearchDetail">
		...
		<dl>
			<dt style="width:55px;">부서</dt>
			<dd><input type="text" autocomplete="off" style="width:170px;" class="puddSetup enter" id="txt_deptName" /></dd>
			<dt style="width:90px;">기안자</dt>
			<dd><input type="text" autocomplete="off" style="width:170px;" class="puddSetup enter" id="txt_aprUserName" /></dd>
			<dt style="width:120px;">장소</dt>
			<dd><input type="text" autocomplete="off" style="width:170px;" class="puddSetup enter" id="txt_location" /></dd>	
		</dl>
		<dl>	
			<dt style="width:55px;">출장자</dt>
			<dd><input type="text" autocomplete="off" style="width:170px;" class="puddSetup enter" id="txt_triper" /></dd>
			<dt style="width:90px;">구분</dt>
			<dd>
	...
	<div class="sub_contents_wrap mb0">						
			<div class="btn_div">
				...
				<div class="right_div">
					<input type="button"  id="btnConsDelete" class="puddSetup"  value="품의서삭제"><!--품의서삭제--></input>
			...
			<div class="down_lay apl_wr" id="apl1" style="display:none;;">
					<div class="down_lay_in">
						<div class="down_lay_con pt5" style="width:100px;">
							<div class="cj_list">
								<ul>
									<li><a href="#n">화면표시 편집</a></li>
								</ul>	
							</div>
						</div><!-- //down_lay_con -->
					</div><!-- // down_lay_in -->
...
<script type="text/javascript">
	...
	function fnDeleteConsDoc(){
		if(!$('.chkData:checked').length){
			alert('삭제할 품의서를 선택하세요.');
			return;
		}
		if(!confirm('삭제된 내역은 복원할 수 없습니다. 삭제하시겠습니까?')){
			return;			
    	}
		
		var tripConsDocs = '';
		$('.chkData:checked').each(function(i, item){
			tripConsDocs +=  ',' + JSON.parse(unescape($(this).attr('data'))).tripConsDocSeq;		
		});
		tripConsDocs = '(' + tripConsDocs.substring(1) + ')';
		
		console.log( 'delete target : ' + tripConsDocs );
		
		$.ajax({
			type : 'post'
			, url : '<c:url value='/trip/admin/report/deleteTripConsDoc.do' />'
			, datatype : 'json'
			, async : false
			, data : {
				tripConsDocSeqs : tripConsDocs
			}
	...
	function fnSetMainGrid(aaData){
		...
		$("#divGridArea").GridTable({
	        ...
	        , "aoHeaderInfo": [{          // [*] 실제 데이터 표기 방법에 대하여 지정합니다.
	        	no: 'A'
	        	, renderValue: '<input type="checkbox" id="allChk"/>' 
	        	, colgroup: '5'
            },{
	            no: '0'
	            , renderValue: "문서번호"
            ...
            , "aoDataRender": [{          // [*] 실제 데이터 표기 방법에 대하여 지정합니다.
                ...
                render: function (idx, item) {
                	if(!item.docNo){
                		/* 진행 중인 결재 필터링 */
                		return "";
                	}else if(!!item.confferCount){
                		/* 참조된 품의서의 경우 필터링 */
                		return "";
                	}
                	return '<input type="checkbox" class="chkData" data="' + escape(JSON.stringify(item)) + '" />';
                }
            }, {          // [*] 실제 데이터 표기 방법에 대하여 지정합니다.
                no: '0',      
                render: function (idx, item) {                        	                     	
                ...
                no: '1',
                render: function (idx, item) {
                	return '<a class="consDoc" style="cursor:pointer" title="품의문서보기">' + item.docTitle + '</a>';
                } ,
                class: 'le'
            }, {
                no: '2',
	...
	function fnSetGrid(){
		$.ajax({
			type : 'post'
			, url : '<c:url value='/trip/admin/cons/selectTripConsReport.do' />'
			, datatype : 'json'
			, async : false
			, data : fnGetReportPageParam()
			, success : function(result) {
				console.log('SERVER CALL >> user cons report RESULT> ');
				console.log(result);
