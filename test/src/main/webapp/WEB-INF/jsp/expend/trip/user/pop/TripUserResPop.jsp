...
<body>
	...
	<div class="pop_sign_head" style="">
		<h1 id="h1_resDocTitle">출장결의 작성</h1>
...
<div class="pop_sign_con">
	<div class="pop_con posi_re mt0">
		...
		<div class="btn_div">
			...
			<div class="right_div">
				<span class="fl mt5 mr10" id="txtConfferConsInfo"></span>
				<input type="button" class="puddSetup" value="출장품의참조" id="btnSelectConsConffer" />
			</div>
		</div>
		...
		<div class="com_ta">
			<table>
				...
				<tr>
					...
					<th>출장 기간</th>
					<td colspan="3">
					<input type="text" autocomplete="off" class="puddSetup" pudd-type="datepicker" value="" id="txt_tripFromDate"/>
				...
				<tr>
					<th>구분</th>
					<td>
						<input type="radio" name="selDomesticRadi" value="0" id="inputDomesticCode0" class="puddSetup selDomesticDiv" pudd-label="국내" checked/>
					...
					<td>
						...
						<input type="text" autocomplete="off" id="txt_locationNote" class="puddSetup"pudd-style="width:260px;" />
					</td>
				</tr>
				<tr>
					<th>일정</th>
				...
				<tr>
					<th>여비지급</th>
					<td>
						<input type="radio" name="selPayRequestRadi" id="inputExpenseCode0" value="0" class="puddSetup selPayRequestDiv" pudd-label="대상" checked />
						<input type="radio" name="selPayRequestRadi" id="inputExpenseCode1" value="1" class="puddSetup selPayRequestDiv" pudd-label="비대상" />
					</td>
					<th>운임수단</th>
					<td>
				...
				<div id="modal_display" class="modal_ab" style="top: 45px;left: 0;right: 0;height: 73px;z-index: 99;display:none;opacity: 0.1;"></div>
			</table>
		</div>

		<!-- 결의정보 ------------------------------------------------------------------------------------------------------------------------->
		<div class="btn_div resSummary">
			<div class="left_div">
				<p class="tit_p mt4 mb0">결의정보</p>	
			</div>
			<div class="right_div">
				<input type="button" class="puddSetup" value="등록" id="btn_resDocOpen" />
			</div>
		</div>

		<div id="grid_resSummaryTable" style="display:none;"  class="resSummary"></div>

		<div class="com_ta4 rowHeight hover_no resSummary">
			<table>
				<colgroup>
					<col width="13.6%"/>
					<col width="18%"/>
...
<script type="text/javascript">
	...
	function fnSetLocationSelect(){
		...
		Pudd( "#sel_location" ).puddComboBox({
			attributes : { style : "width:148px;" }// control 부모 객체 속성 설정
		,	controlAttributes : { id : "exAreaSelectBox" }// control 자체 객체 속성 설정
		,	dataSource : dataSourceComboBox
			...
			, success : function(result) {
				if(result.resultMessage = 'SUCCESS'){
					...
					for(var i=0; i < result.result.calList.length; i++){
						...
						if(!!optionSet.tripResDoc.confferDocSeq){
							/* 참조품의 결의의 경우 선택한 캘린더 리스트만 노출*/
							if( item.mcalSeq == calendarSeq ){
								filterdList.push(item);
							}
						}else if (!optionSet.tripResDoc.confferDocSeq){
	...
	function fnSetEventInit(){
		...
		$(".pbud").click(function(){
			if ($(".pbud").hasClass("pudd_btn_down")) {
				...
				$("#grid_resSummaryTable").slideDown(300);
			} else {
				$(".pbud").removeClass("pudd_btn_up");
				$(".pbud").addClass("pudd_btn_down");
				$("#grid_resSummaryTable").slideUp(300);
			}
		});
		
		/* 결의정보 입력 진행 */
		$('#btn_resDocOpen').click(function(){
			/* 결의 정보 입력 */
			fnResDocOpen();
		});
		...
		$('#btnSelectConsConffer').click(function (){
			...
			var pop = window.open('', "tripConsConfferPop", "width=1000, height=513, left=150, top=150");
			
			confferPop.target = "tripConsConfferPop";
			confferPop.method = "post";
			confferPop.action = url;
			confferPop.submit();
			confferPop.target = "";
			pop.focus();
		});
	...
	/*	[콜백] 참조품의 가져오기 콜백 
	----------------------------------------------------- */
	function fnSelectConsConfferReturn(aData){
		aData.tripResDocSeq = optionSet.tripResDoc.tripResDocSeq;
		$('#btn_resDocOpen').val('수정');
		console.log('call Conffer cons callback aData . >> ');
		console.log(aData);
		$.ajax({
			type : 'post'
			, url : "<c:url value='/trip/user/res/updateTripConfferCons.do' />"
			, datatype : 'json'
			, async : false
			, data : aData
			, success : function(result) {
				...
				console.log(result);
				if(result.result.resultCode != 'SUCCESS'){
					alert('작성된 품의서 정보 반영 실패');
					return;
				}else{
					optionSet.tripResDoc.confferDocSeq = aData.consDocSeq;
					optionSet.tripResDoc.tripConsDocSeq = aData.tripConsDocSeq;
					$('#txtConfferConsInfo').text('참조 품의 : ' + aData.docNo );
					$('#btnSelectConsConffer').hide();
					$('#btn_attendReload').hide();
					$('#btn_userSelect').attr('disabled', true);
					fnSetDisplayResData(result.result.aData);
					$('#select_org_info').val(JSON.stringify(result.result.aData.travelerResult));
				}
			}, error : function(result) {
				console.error(result);
			}
	...
	/*	[참조품의] 참조품의 가져오기 기능 반영 */
	function fnSetDisplayResData(item){
		var resDocResult = item.resDocResult;
		var attendData = item.attendResult[0];
		var expenseData = item.amtResult; 
		var	travelerData = item.travelerResult;
		var resData = {
			mgtData : item.mgtResult
			, bgtData : item.budgetResult
			, resAmt : item.resAmt || 0
		}; 

		/* 참조품의 반영여부 확인 */
		if( (!!resDocResult) && (!!resDocResult[0].confferDocSeq)){
			optionSet.tripResDoc = resDocResult[0];
			$('#txtConfferConsInfo').text('참조 품의 : ' + resDocResult[0].confferDocNo );
			$('#btnSelectConsConffer').hide();
		}
		
		/* 여비지급 대상여부 (대상) / 결의서 등록 여부 확인 */
		if(item.attendResult[0].tripExpenseCode == '0' && (!!optionSet.tripResDoc.resDocSeq)){
			$('#btn_resDocOpen').val('수정');
			$('#resDocSeq').val(optionSet.tripResDoc.resDocSeq);
		}
		
		/* 결의정보 출력 */
		if(!!resData){
			fnSetResSummarys(resData);
		}
		...
		/* 국내외 구분 콜백 */
		$('#inputDomesticCode' + attendData.domesticCode).next().next().click();
		...
		/* 여비지급 대상여부 변경 */
		$('#inputExpenseCode' + attendData.tripExpenseCode).next().next().click();
		...
		preFromDate = $('#txt_tripFromDate').val();
		preToDate = $('#txt_tripToDate').val();
		
		/* 시간차 정보 재설정 */
		fnSetDisplayTimeGap();
		if(!optionSet.tripResDoc.tripConsDocSeq){
			$('#modal_display').show();	
		}
		/* 경비내역 정보 */
			...
			, success : function(result) {
				if(result.result.resultCode == 'SUCCESS'){
					...
					for(var i=0; i < optionSet.transport.length; i++ ){
						...
						if(transportSeqs.split(',').indexOf( ('' + (item.transportSeq || '-1')) ) > -1){
							item.selected = true;
	...
	function fnDraftApproval(){
		...
		$.ajax({
			type : 'post'
			, url : '<c:url value='/trip/user/res/insertTripRes.do' />'
			, datatype : 'json'
			, async : false
			, data : param
			, success : function(result) {
				console.log('		[res info] 지출 품의 2.0 작성 데이터 >');
				console.log(result);
				if(result.result.resultCode != 'SUCCESS'){
					alert('작성된 결의서 정보 저장 실패');
					return;
				}else{
					result2 = result;
					/* 전자결재 작성 */
	...
	function fnDraftApprovalPreCheck(){
		...
		if(isChangeDate == '1'){
			alert('참조품의시 품의시 설정된 출장일수는 변경 될 수 없습니다.');
			return false;
		}
		
		if($('input:radio[name=selPayRequestRadi]:checked').val() == '0'){
			if(!parseInt($('#txt_mgtCnt').html())){
				alert('결의정보를 입력해 주시기 바랍니다.');
				$('#btn_resDocOpen').click();
				return false;
			}
			
			if(!fnJudgeAmt()){
				alert('결의합계 금액과 경비합계 금액이 일치하도록 금액을 조정해 주세요.');
				return false;
			}
		}
		/* else{
			if(!!optionSet.tripResDoc.confferDocSeq){
				alert('여비지급 대상인 출장품의서를 비대상으로 상신할 수 없습니다.');
				return false;
			}
		} */
		return true;
	...
	function fnEventApproval() {
		...
		var resDocSeq = $('input:radio[name=selPayRequestRadi]:checked').val() == '0' ? optionSet.tripResDoc.resDocSeq : '0';
		parameter.approKey = optionSet.formInfo.formDTp + '_TRIP_' + optionSet.tripResDoc.tripResDocSeq + '_' + ( resDocSeq || '0' );
		parameter.tripResDocSeq = optionSet.tripResDoc.tripResDocSeq;
		parameter.tripConsDocSeq = optionSet.tripResDoc.tripConsDocSeq;
		parameter.resDocSeq = optionSet.tripResDoc.resDocSeq;
	...
	/*	[품의정보] 출장 품의 정보 입력
	-------------------------------------------------- */
	function fnResDocOpen(){
		/* 출장결의 결의서 정보 입력 */
		var url = "<c:url value='/expend/np/user/NpUserCRDocPop.do'/>" + '?formSeq=' + optionSet.formInfo.formSeq;
		var pop = window.open('', "resDocPop", "width=" + 900 + ", height=" + 900 + ", left=" + 150 + ", top=" + 150);
		$('#tripDocSeq').val(optionSet.tripResDoc.tripResDocSeq);
		$('#confferDocSeq').val(optionSet.tripResDoc.confferDocSeq);
		
		resDocPop.target = "resDocPop";
		resDocPop.method = "post";
		resDocPop.action = url;
		resDocPop.submit();
		resDocPop.target = "";
		pop.focus();	
	}
	...
	/*	[출장 정보] 결의서 정보 입력 콜백
	-------------------------------------------------- */
	function fnResDocPopCallback(param){
		optionSet.tripResDoc.resDocSeq = param.resDocSeq;
		$('#btn_resDocOpen').val('수정');
		$('#resDocSeq').val(optionSet.tripResDoc.resDocSeq);
		/* 결의문서 정보 조회 */
		$.ajax({
			type : 'post'
			, url : '<c:url value='/trip/user/res/selectResDocInfo.do' />'
			, datatype : 'json'
			, async : false
			, data : param
			, success : function(result) {
				console.log('		[res info] 지출 결의 2.0 작성 데이터 >');
				console.log(result);
			
				if(result.result.resultCode != 'SUCCESS'){
					alert('작성된 결의서 정보 조회 실패');
					return;
				}else{
					fnSetResSummarys(result.result.aData);
					fnSetResBudgetInfo(result.result.aData);
				}
			}, error : function(result) {
				console.error(result);
			}
	...
	/* 수정 후 예산 및 과목 정보 저장*/
	function fnSetResBudgetInfo(aData){
		aData.bgtData[0]||{resDocSeq:''}
		var data = {
				bgtData : JSON.stringify(aData.bgtData)
				, mgtData :	JSON.stringify(aData.mgtData)
				, consAmt : aData.consAmt
				, tripDocSeq : $('#tripDocSeq').val()
				, resDocSeq : aData.bgtData[0].resDocSeq
				, isReWrite : isReWrite
		}
		/* 품의문서 예산,과목 정보 저장 */
		$.ajax({
			type : 'post'
			, url : '<c:url value='/trip/user/res/insertTripResBudget.do' />'
			, datatype : 'json'
			, async : false
			, data : data
			, success : function(result) {
				console.log('		[cons info] 지출 결의 2.0 작성 예산 데이터 >');
				console.log(result);
				isReWrite = 1;
	...
	/*	[결의정보] 결의정보 테이블 정보 출력
	-------------------------------------------------- */
	function fnSetResSummarys(aData){
		/* 요약 정보 출력 */
		aData.mgtData = aData.mgtData || [];
		aData.bgtData = aData.bgtData || [];
		
		$('#txt_mgtCnt').text(aData.mgtData.length);
		$('#txt_bgtCnt').text(aData.bgtData.length);
		$('#txt_totalAmt').text( fnGetCurrencyFormat( aData.resAmt ) );
		...
		$("#grid_resSummaryTable").empty();
		if(optionSet.conVo.erpTypeCode == 'ERPiU'){
			Pudd("#grid_resSummaryTable").puddGrid({
			...
			,	columns : [
					{
						...
						,	content : {
								template : function( rowData ) {
									if(rowData.tempSelect == 'Y'){
										if(optionSet['gw']['3']['3'].setValue == '0'){
											var d = new Date();
											return d.getFullYear() + '-' + (9 < (d.getMonth() + 1) ? (d.getMonth() + 1) : ('0' + (d.getMonth() + 1))) + '-' + (9 < d.getDate()? d.getDate():'0'+d.getDate());
										}
									}
									return fnGetDateFormat(rowData.resDate);
								}
							}
					}
			,	{
	...
	-------------------------------------------------- */
	var _expUseTemp;
	function fnSetResSummaryDisplay(){
	...
	function fnSetDisplayTimeGap(){
	    ...
	    if($('#txtConfferConsInfo').text()!=''){
	    	var preFrArr = preFromDate.split('-');
		    var preToArr = preToDate.split('-');
		    
		    var preDiffDate_from = new Date(preFrArr[0],preFrArr[1]-1,preFrArr[2]);
		    var preDiffDate_to = new Date(preToArr[0],preToArr[1]-1,preToArr[2]);
		    
		    var preDiff = Math.abs(preDiffDate_to.getTime() - preDiffDate_from.getTime());
		    preDiff = Math.ceil(preDiff / (1000 * 3600 * 24)) + 1;
	    ...
	    }else if(isChangeDate == '1'){
	    	$('#txt_alertOverTime').html('참조품의시 출장일수가 변경될 수 없습니다.');
	    } else{
	...
	function fnCreateTripResDoc(){
		...
		if(!approKey){
			...
			/* 이전단계 결재 정보 조회 */
			fnCreateReResDoc(approKey);
		}
	}
	
	/*	[Redraft trip cons doc] 출장 결의 문서 이전단계 로직 시작
	-------------------------------------------------- */
	function fnCreateReResDoc(approKey){
		...
		param.tripResDocSeq = approKeyArray[2].toString();
		param.resDocSeq = approKeyArray[3].toString();
		optionSet.tripResDoc = { tripResDocSeq : approKeyArray[2].toString(), resDocSeq : approKeyArray[3].toString() };
		$.ajax({
			type : 'post'
			, url : '<c:url value='/trip/user/res/selectTripResDocAllInfo.do' />'
			, datatype : 'json'
			, async : false
			, data : param
			, success : function(result) {
				console.log('SUCCESS SELECT RES DOC - ');
				console.log('RES DOC INFO >>> ');
				if(result.result.resultCode == 'SUCCESS'){
					$('#select_org_info').val(JSON.stringify(result.result.aData.travelerResult));
					setTimeout(fnSetDisplayResData, 500, result.result.aData);
				}else{
					alert('결의정보 조회에 실패하였습니다.');
				}
			}, error : function(result) {
				console.error(result);
			}
	...
	-------------------------------------------------- */
	function fnCreateNewResDoc(){
		var param = cmFnc_getUserInfo();
		param.formSeq = optionSet.formInfo.formSeq;
		
		$.ajax({
			type : 'post'
			, url : '<c:url value='/trip/user/res/insertTripResDoc.do' />'
			, datatype : 'json'
			, async : false
			, data : param
			, success : function(result) {
				console.log('SUCCESS CREATE RES DOC - ');
				console.log('RES DOC INFO >>> ');
				console.log(result)
				if(result.result.resultCode == 'SUCCESS'){
					$('#h1_resDocTitle').html($('#h1_resDocTitle').text() + ' / ' + result.result.aData.tripResDocSeq);
					optionSet.tripResDoc = {tripResDocSeq : result.result.aData.tripResDocSeq};
				}else{
					alert('결의서 생성에 실패하였습니다.');
...
,				pageSize : 500
	...
	-------------------------------------------------- */
	function fnGetTransportSelectedData(){
	...
	function fnSelectAmtDatas(users){
		...
		param.transportSeq = fnGetTransportSelectedData();
		...
		$.ajax({
			...
			, success : function(result) {
				if(result.result.resultCode == 'SUCCESS'){
					/* var sortedOrgData = fnSortSelectedOrgInfo(result.result.aaData);
					$('#select_org_info').val(JSON.stringify(sortedOrgData)); */
	...
	/* function fnSortSelectedOrgInfo(optionAmtData){
		var aaData = optionAmtData;
		var orgInfos = JSON.parse($('#select_org_info').val());
		var empSeqs = [];
		var orgTempInfo = [];
		for(var aData of aaData){
			empSeqs.push(aData.empSeq);
		}
		
		for(var empSeq of empSeqs){
			for(var orgInfo of orgInfos){
				 if(orgInfo.empSeq == empSeq){
					 orgTempInfo.push(orgInfo);
				 }
			}
		}
		...
		Pudd("#grid_expenseDetailTable").puddGrid({
			...
			,	columns : [ {
					...
					, content : {
						template : function( rowData ) {
							...
							var amt = '';
							if( (!!optionSet.tripResDoc.tripConsDocSeq) ){
								amt = rowData.amtClass1Amt || '';
							} else if( ( !'${param.approKey}' ) ){
					...
					, content : {
						template : function( rowData ) {
							...
							if( (!!optionSet.tripResDoc.tripConsDocSeq) ){
								amtSum = parseInt(rowData.amtClass1Amt) + parseInt(rowData.amtClass2Amt) + parseInt(rowData.amtClass3Amt) + parseInt(rowData.amtClass4Amt) + parseInt(rowData.amtClass5Amt) + parseInt(rowData.amtClass6Amt);
							}  
							else if(!'${param.approKey}'){
								amtSum = fnGetDailAmt('1', rowData.amtClass1Amt || '0' ) + fnGetDailAmt('2', rowData.amtClass2Amt || '0' ) + fnGetDailAmt('3', rowData.amtClass3Amt || '0' ) + parseInt(rowData.amtClass4Amt) + parseInt(rowData.amtClass5Amt) + parseInt(rowData.amtClass6Amt);
							} 
							else {
								if(isClear=='1'){
									amtSum = fnGetDailAmt('1', rowData.amtClass1Amt || '0' ) + fnGetDailAmt('2', rowData.amtClass2Amt || '0' ) + fnGetDailAmt('3', rowData.amtClass3Amt || '0' ) + parseInt(rowData.amtClass4Amt) + parseInt(rowData.amtClass5Amt) + parseInt(rowData.amtClass6Amt);	
			...
			, loadCallback : function( headerTable, contentTable, footerTable, gridObj ) {
				...
				$('.amtMask').keyup(function(){
					...
					} else if ((!!optionSet.tripResDoc.tripConsDocSeq)  && thisAmt > maxAmt){
						/* 품의 참조한 결의서의 경우 품의금액을 넘을 수 없다. */
						$(this).val(maxAmt);
					}else if(thisAmt === 0){
						$(this).val('');
					}
				...
				for( var i = 0; i < contentTable.rows.length; i++ ){
					...
					if( (!!optionSet.tripResDoc.tripConsDocSeq) ){
						/* 참조품의 가져오기 자동 계산 */
						totalAmt1 += parseInt( item.amtClass1Amt );
						totalAmt2 += parseInt( item.amtClass2Amt );
						totalAmt3 += parseInt( item.amtClass3Amt );
					}  else if(!'${param.approKey}'){
						/* 일반 작성 자동계산 */
...
<form id="resDocPop" name="resDocPop" method="post">
	<input type="hidden" name="EXNPInterlockAPI" id="EXNPInterlockAPI" value="1"/>
	<input type="hidden" name="EXNPInterlockFormDTP" id="EXNPInterlockFormDTP" value="RES"/>
	<input type="hidden" name="EXNPInterlockFormDTP2" id="EXNPInterlockFormDTP" value="TRIPRES"/>
	<input type="hidden" name="tripDocSeq" id="tripDocSeq" value=""/>
	<input type="hidden" name="confferDocSeq" id="confferDocSeq" value=""/>
	<input type="hidden" name="resDocPopCallback" id="resDocPopCallback" value="fnResDocPopCallback"/>
	<input type="hidden" name="resDocSeq" id="resDocSeq" value=""/>
</form>	

<form id="confferPop" name="frmPop" method="post">
	<input type="hidden" name="formSeq" id="" value="1"/>
