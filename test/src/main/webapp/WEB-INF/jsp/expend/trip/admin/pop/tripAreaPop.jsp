...
<body>
	<div class="pop_wrap">
<%-- 		<div class="pop_head">
			<h1><%=BizboxAMessage.getMessage("TX000004662","출장지", langCode)%> <%=BizboxAMessage.getMessage("TX000019777","선택", langCode)%><c:if test="${domesticDiv == 'L' }">(<%=BizboxAMessage.getMessage("TX000015728","국내", langCode)%>)</c:if><c:if test="${domesticDiv == 'F' }">(<%=BizboxAMessage.getMessage("TX000020684","국외", langCode)%>)</c:if></h1>
			<a href="#n" class="clo"><img src="${pageContext.request.contextPath}/resources/Images/btn/btn_pop_clo02.png" alt="" /></a>
		</div>			
 --%>			
		<div class="pop_con">
			<div class="top_box">
...
function fnInitClickEvent(){
	...
	$("#confirmButton").click(function () {		 
		fnCallPopupCallback(selectedLocation);
    });
	...
	Pudd( "#grid" ).on( "gridRowClick", function( e ) {
		...
		selectedLocation = evntVal.trObj.rowData;
	});
...
function fnCallPopupCallback( param ){
	var callbackFuncName = 'setLocationCallback';
	window.opener[callbackFuncName](param);
	self.close();
}


function fnGridSetLocationTable(){
	...
	var LocationData = new Pudd.Data.DataSource({
		 request : {
			...
			,	parameterMapping : function( data ) {
					...
					data.domesticDiv = $("#domesticDiv").val();
					data.useYn = 'Y';
				}
		}, result : {
