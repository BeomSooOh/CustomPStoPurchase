...
<div id="reldiv2">
	<br />
	<span class="bold">EXP 사용자 메인</span>
</div>
<style>
span.bold {
	font-weight: bold;
...
<script type="text/javascript">
    $(window).load(function() {
        var iframes = $("iframe");
        var page = [ "notice", "poll", "main_banner", "Mbanner_L", "Mbanner_R", "M_approval", "M_defer", "http://221.133.55.230/schedule/Views/Common/schedule/scheduleMain", "http://221.133.55.230/schedule/Views/Common/note/noteMain", "quick" ];
        for (var i = 0; i < iframes.length; i++) {
            var iframe = iframes[i];
            if (i == 0) {
                $(iframe).attr("src", "userInfo.do");
            } else {
                var rex = /^((http(s?))\:\/\/)([0-9a-zA-Z\-]+\.)+[a-zA-Z]{2,6}(\:[0-9]+)?(\/\S*)?$/;

                var url = page[i - 1];

                if (rex.test(url)) {
                    $(iframe).attr("src", page[i - 1]);
                } else {
