...
//         dataBound: gridDataBound,
        columns: [
                ...
                ,{   field:"DOCU_MODE_NM",title: "문서분류",width:100,headerAttributes: {style: "text-align: center;"}
                     ,attributes: {style: "text-align: center;"}
                }
                ,{   field:"ERP_GISU_DT",title:"결의일자",width:100,headerAttributes: {style: "text-align: center;"},attributes: {style: "text-align: center;"}}
                ,{   field:"ERP_GISU_SQ",title:"결의번호",width:70,headerAttributes: {style: "text-align: center;"},attributes: {style: "text-align: center;"}}
                ,{   field:"DOCU_FG_NM",title:"결의구분",width:120,headerAttributes: {style: "text-align: center;"},attributes: {style: "text-align: center;"} }
                ,{   field:"MGT_NM",title:"프로젝트",width:150,headerAttributes: {style: "text-align: center;"},attributes: {style: "text-align: center;"} }
                ,{   field:"APPLY_AM",title:"집행액",width:73,headerAttributes: {style: "text-align: center;"},attributes: {style: "text-align: right;"} }
                ,{   field:"C_RIDOCFULLNUM",title:"문서번호",width:100,headerAttributes: {style: "text-align: center;"},attributes: {style: "text-align: center;"} }
                ,{   field:"USERNAME",title:"기안자",width:100,100: {style: "text-align: center;"},attributes: {style: "text-align: center;"} }
                ,{   field:"DOC_STATUS",title:"결재상태",width:100,headerAttributes: {style: "text-align: center;"},attributes: {style: "text-align: center;"} }
                ,{   field:"DOCTITLE", title: "문서제목", headerAttributes: {style: "text-align: center;"},template: "<a href=javascript:docList.docView('#=C_DIKEYCODE#');>#: data.DOCTITLE# </a>"   }
            ],
...
docList.docDelete = function(){
    ...
    if (checkList[0].APPR_STATUS < '008'){
    	alert('결재완료된 문서만 삭제할수 있습니다.\n삭제를 원하시면 결재회수 하시기 바랍니다.');  return;	
    }
    
    if (checkList[0].APPR_STATUS == 'd'){
        alert('이미 삭제되었습니다.');  return; 
    }
    	...
    	$.ajax({
            ...
            success:function(data){
                ...
                if(checkList[0].DOCU_MODE == '1' && data.result.StatChk > 0 ){
                    alert("전표가 생성되었습니다.\n ERP 에서 전표삭제후 진행해 주세요.");
                    return;
                }
                
                if(checkList[0].DOCU_MODE == '0' && data.result.StatChk > 0 ){
                    alert("이미 참조품의 결의서가 작성되었습니다. 품의서를 삭제할수 없습니다. .");
                    return;
                }
                
                if(data.result.rollbak){
                	alert("참조품의서의 환원처리를 취소하였으니 확인바랍니다.")
                	docList.searchList();
                }
...
<!--                                             <input class="k-textbox input_search" id="" type="text" value="" style="width:100px;" placeholder=""> -->
                                ...
                                <tr>
                                    ...
                                    <td><input id="SET_FG" name="SET_FG" style="width:100px;" class="formData"/>
                                        <span class="btn_Detail btn_Detail_no">상세 <img id="all_menu_btn" src="<c:url value='/Images/ico/ico_btn_arr_down01.png'/>"/></span>
                                    </td>
                                </tr>
                            </table>            
                        </div> 
...
<%--                            <c:if test="${delAuthor == 'Y'}" > --%>
                           <button type="button"  onclick="docList.docDelete();">문서삭제</button>
<%--                            </c:if> --%>
                        </div>                
