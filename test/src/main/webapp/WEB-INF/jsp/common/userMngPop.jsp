...
<form id="USER_mngPop" name="frmPop" method="post">
...
<script>
	...
	function CM_FNC_VALIDATOR_CARD(param) {
		try {
			...
			console.log('[ * EXP ] 항목관리 함수호출 오류 : ' + exMsg);
			return exMsg;
		}
		return 0;
	}
	...
	function CM_FNC_CALL_POPUP_CARD(param) {
		// 경로 고정.
		var url = "<c:url value='/ex/user/UserMngPop.do'/>" + '?comp_seq=6';
		var pop = window.open("", "UserMngPop",
				"width=700,height=578,scrollbars=no");
	
		USER_mngPop.target = "UserMngPop";
		USER_mngPop.method = "post";
		USER_mngPop.action = url;
		USER_mngPop.submit();
		USER_mngPop.target = "";
		pop.focus();
	}
