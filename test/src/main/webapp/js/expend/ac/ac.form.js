...
abdocu.state = function ( state ) {
	if ( state ) {
		...
		var img = $ ( "<img>" ).attr ( "id", "g20-ajax-img" ).attr ( "src", _g_contextPath_ + "/Images/ico/loading.gif" ).css ( "position", "absolute" ).css ( "top", "50%" ).css ( "left", "50%" ).css ( "margin-top", "-5" ).css ( "margin-left", "-5" ).css ( "zIndex", "100000" );
		$ ( "body" ).append ( img );
	} else {
...
function fnAbdocuInit ( ) {
	...
	$.ajax ( {
		...
		success: function ( res ) {
			...
			console.log(res.result.aData);
			console.log(res.result.aData.erpEmpInfo);
			console.log(res.result.aData.erpGisuListInfo);
			
			var erpEmpInfo = (res.result.aData.erpEmpInfo || "");
			var erpGisuListInfo = (res.result.aData.erpGisuListInfo || "");
			
			/* 전자결재 양식 명칭 표시 */
			// $ ( ".title_NM" ).html(erpEmpInfo.erpCompName + '(' + erpEmpInfo.erpCompSeq + ')');
			
			/* 연동 ERP 회사정보 표시 */
			$ ( "#coInfo" ).html ( erpEmpInfo.erpCompName + ' (' + erpEmpInfo.erpCompSeq + ')' );
...
function fnInputInit ( res ) {
	...
	if ( abdocuInfo.abdocu_no != 0 ) {
		...
		$ ( "#txt_Memo" ).val ( abdocuInfo.rmk_dc );

		// 원인행위 등록 옵션
		if ( erpOption.CauseUseYn == "1" && abdocuInfo.docu_mode == "1" ) {
			$ ( "#causeForm" ).show ( );
			...
			if ( !abdocuInfo.cause_id ) {
				$ ( "#CAUSE_ID" ).attr ( "CODE", "empty" );
			}
			if ( !abdocuInfo.cause_nm ) {
...
abdocu.append = function ( type ) {
	...
	if ( !$ ( ".erpappend", temptd ).length ) {
		...
		erpappend.click ( function ( ) {
			...
			if ( !acUtil.util.validationCheck ( table ) ) { return false; }
...
abdocu.init = function ( ) {
	/**
	 * 참조품의가져오기 한 지출결의서가 아닐경우(일반적인경우) 프로젝트 선택 가능하게~
	 */
	if ( !abdocu.isReffer ( ) ) {
...
/*
 * 03. 예산정보 시작 abdocu_no가 존재할경우 하위 예산정보 목록 불러와서 화면에 보여준다. noInitSet : 첫번째 row 의 거래처정보 셋팅 할껀지 여부 true : select 하지 않음 false : 0 번째 로우에 select
 */
	...
	if ( !abdocu.isReffer ( ) ) {
		...
		$ ( ".txt_BUDGET_LIST", context ).bind ( {
			dblclick: function ( ) {
				...
				var id = $ ( this ).attr ( "id" );
				$.eventEle = id;
				var dblClickparamMap = ( function ( ID, idx ) {
					var returnObj = [ {
						"id": id,
						"text": "BGT_NM",
						"code": "BGT_CD"
...
abdocu.BudgetInfo.focusNextRow = function ( eventEle ) {
	...
	var table = $ ( "#erpBudgetInfo-table" );
	if ( !acUtil.util.validationCheck ( table, eventEle ) ) { return false; }
	var resultData = {};
...
/*
 * - 호출되는 경우 1. 예산정보(중간) 에 따른 하위 거래처정보 목록 조회 2. 예산정보에서 다른 row(행)을 선택했을경우 거래처 다시 binding(DB조회)
 */
...
abdocu.TradeInfo.RowSave = function ( eventEle, resultData ) {
	...
	saveObj.tr_nm = $ ( ".txt_TR_NM", parentEle ).val ( ) || ""; /* 거래처명 */
	...
	saveObj.btr_nm = $ ( ".txt_BTR_NM", parentEle ).val ( ) || ""; /* 금융기관 */
	saveObj.btr_cd = $ ( ".txt_BTR_NM", parentEle ).attr ( "CODE" ) || "";/* 금융기관 */
	saveObj.jiro_nm = $ ( ".txt_JIRO_NM", parentEle ).val ( ) || ""; /* 금융기관 */
	saveObj.jiro_cd = $ ( ".txt_JIRO_CD", parentEle ).attr ( "CODE" ) || ""; /* 금융기관 */
	saveObj.depositor = $ ( ".txt_DEPOSITOR", parentEle ).val ( ) || ""; /* 예금주 */
	...
	saveObj.ba_nb = $ ( ".txt_BA_NB", parentEle ).val ( ) || ""; /* 계좌번호 */
	...
	saveObj.tr_fg = $ ( ".txt_TR_FG", parentEle ).val ( ) || ""; /* erp data */
	saveObj.tr_fg_nm = $ ( ".txt_TR_FG_NM", parentEle ).val ( ) || ""; /* erp data */
...
abdocu.TradeInfo.addRow = function ( ) {
	...
	$ ( ".txt_UNIT_AM", tr ).bind ( {
		...
		keyup: function ( event ) {
			...
			if ( abdocuInfo.docu_mode == "0" && MakeTradeTable.getModeType ( abdocuInfo.docu_mode, selectDocu ) == "4" ) { return; }
			...
			if ( abdocuInfo.docu_mode == "1" && modeType == "9" ) {
				...
				if ( intxAM < 1000 ) {
					...
					intxAM = ( ( Math.round ( intxAM ) ) );
					intxAM = Math.round ( intxAM, 1 );
					var rstxAM = ( ( Math.round ( parseInt ( intxAM, 10 ) * 0.1 ) ) );
					rstxAM = Math.round ( rstxAM, 1 );
	...
	$ ( ".txt_VAT_AM", tr ).bind ( {
		...
		keyup: function ( event ) {
			...
			if ( abdocuInfo.docu_mode == "0" && ( modeTypeVal == "1" || modeTypeVal == "4" ) ) { return; }

			if ( abdocuInfo.docu_mode == "1" && ( modeType == "4" || modeType == "8" ) ) { return; }
	...
	$ ( ".txt_SUP_AM", tr ).bind ( {
		...
		keyup: function ( event ) {
			...
			/* 품의서 기타 소득자 일때는 작동 안함 */
			if ( abdocuInfo.docu_mode == "0" && modeTypeVal == "4" ) { return; }

			/* 결의서 기타 소득자, 사업소득자, 급여 일때는 작동 안함 */
			if ( abdocuInfo.docu_mode == "1" && ( modeType == "4" || modeType == "9" ) ) { return; }
...
abdocu.TradeInfo.getETCPopSet = function ( parentEle ) {
	...
	if ( CTD_CD > "70" ) {
		...
		if ( $ ( ".txt_DATA_CD", parentEle ).val ( ) == "G" ) {
			$ ( "#etc_percent" ).removeAttr ( "disabled" ).removeClass ( "input_gray" );
			$ ( "#etc_percent" ).val ( $ ( ".txt_ETCRATE", parentEle ).val ( ) );
		}
	} else {
		$ ( "#etc_percent" ).val ( "" );
	}
...
abdocu.DateValidate = function ( dateStr ) {
	...
	if ( dateStrArr.length != 3 ) {
		...
		if ( parseInt ( dateStrArr [ 0 ], 10 ) && parseInt ( dateStrArr [ 1 ], 10 ) && parseInt ( dateStrArr [ 2 ], 10 ) && dateStrArr [ 0 ].length == 4 && dateStrArr [ 1 ].length == 2 && dateStrArr [ 2 ].length == 2 ) {
...
abdocu.approvalValidation = function ( ) {
	...
	$ ( ".requirement", table ).each ( function ( ) {
		if ( !$ ( this ).val ( ) || !$ ( this ).attr ( "CODE" ) ) {
			alert ( $ ( this ).attr ( "id" ) );
			alert ( $ ( this ).attr ( "class" ) + " : " + $ ( this ).val ( ) + " : " + $ ( this ).attr ( "CODE" ) );
	...
	for ( var i = 0, max = ids.length; i < max; i++ ) {
		...
		var opt = {
			...
			successFn: function ( data ) {
				...
				if ( data.ctlFg < 5 ) { return; }
				if ( isNaN ( result ) ) {
					result = -1;
				}
				if ( result < 0 ) {
					overBudget [ overBudget.length ] = ids [ i ];
				}
			}
		};
		/* 결과 데이터 담을 객체 */
...
function fnACardSunginPop ( ) {
	...
	acLayerPopOpen ( url, tblParam, "828", "", "ACardSunginPop" );
