...
(function ($) {
	...
	var methods = {
		...
		colGetData: function (r, c) {
			...
			if(r >= 0){
				if (options.columns[c].type == alpha.vars.type.text || options.columns[c].type == alpha.vars.type.autocomplete || options.columns[c].type == 'date' || options.columns[c].type == 'time') {
		...
		setCellMod: function () {
			...
			if (colType === alpha.vars.type.checkbox) {
				...
				options.selectedCell.find('input').keydown(function (event) {
					if(options.columns[c].editor){
						if([113, 13, 37, 39, 38, 40].indexOf((event.keyCode || event.which)) === -1){
							return;
						}
					}
