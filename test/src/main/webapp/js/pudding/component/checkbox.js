/**
* @projectDescription PUDD( 가칭 ) Component CHECK BOX
*
...
(function ($) {
	...
	$.fn.PuddCheck = function( opt ){
		$.each( $( this ), function(index, item){
			...
			//체크박스 생성
			PuddManager.makeComponent( $( this ), puddCheckBox );
		});
	};
	...
	ComponentCommander.checkbox.prototype.getValue = function( selector ){
		if( $( selector ).length <= 1){
			return var json = { 'html' : $( selector )[ 0 ].outerHTML, 'id' : $( selector ).attr( 'id' ), 'class' : $( selector ).attr( 'class' ), 'name' : $( selector ).attr( 'name' ) , 'value' : $( selector ).val( ) };
		}else{
			var retValue = [ ];
			$.each( $( selector ), function( index, item ){
				var json = { 'html' : $( item )[ 0 ].outerHTML, 'id' : $( item ).attr( 'id' ), 'class' : $( item ).attr( 'class' ), 'name' : $( item ).attr( 'name' ) , 'value' : $( item ).val( ) };
				retValue.push( json );
			});
			return retValue;
		}
	...
	ComponentCommander.checkbox.prototype.getChecked = function( selector ){
		if( $( selector ).length <= 1 ){
			...
			$( selector ).each(function( index, item ) {
				if( $( selector ).is( ':checked' ) ){
					var json = { 'html' : $( item )[ 0 ].outerHTML, 'value' : $( item ).val( ) };
					retValue.push( item );
				}
			});
			return retValue;
	...
	PuddCheckBox.prototype = {
		...
		setElementAssembly : function( ){
			...
			if( this.option.dash != undefined && this.option.dash == true ){
		...
		setElementStyle : function( ){
			puddCheckBoxStyle.pushDivStyle( this.$DIV, this.option, this.outStyle );
			if( this.INPUT != null ){
				puddCheckBoxStyle.pushInputStyle( this.$INPUT, this.option, this.inStyle );
			}
		},
		...
		setElementAttribute : function( ){
			...
			if( this.INPUT != null){
				puddCheckBoxAttribute.pushInputAttribute( this.$INPUT, this.option, this.id );
			}
...
CheckBoxEvent.prototype.CheckBoxChecked = function( eventJson ){
	...
	$('.PUDD-UI-checkbox').on('click',function(){
		...
		var $Condition = null;
		multiCheckCount = (multiCheckCount == undefined ? 1 : multiCheckCount );
