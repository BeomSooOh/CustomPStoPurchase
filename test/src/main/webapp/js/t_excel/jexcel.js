...
var JExcel = function( defaultFont ) {
	...
	this.generate = function( filename ) {
		...
		zip.generateAsync( { type: "blob", compression: "DEFLATE" } ).then( function( content ) { saveAs( content, filename ); } );	// And generate !!!
	};
	...
	this.generateRows = function( rows ) {
		...
		for( var index = 0; index < rows.length; index++ ) {
			...
			if( rows[ index ] ) {
				...
				oRows.push( this.generateRow( rows[ index ], index ) );
			}
		}
