		...
		try{
			...
			for(Map<String, Object> item : erpTotalAmtList){
				...
				if(erpTotalAmtMap.containsKey( cdBgacct )){
					int totalAmt = CommonConvert.NullToInt( item.get( "AM_AMTSUM" ) ) + CommonConvert.NullToInt( erpTotalAmtMap.get( cdBgacct ) );
			...
			for(Map<String, Object> item : erpApplyAmtList){
				...
				if(erpApplyAmtMap.containsKey(CommonConvert.NullToString(item.get("CD_BGACCT")))) {
					erpAmt = (int) erpApplyAmtMap.get( CommonConvert.NullToString( item.get( "CD_BGACCT" )) );
					erpAmt += CommonConvert.NullToInt( item.get("AM_DRCR") );
				}
				else {
					erpAmt = CommonConvert.NullToInt( item.get("AM_DRCR") );
				}
			...
			if( CommonConvert.NullToString( params.get( "advAmtOption" ) ).equals( "2" ) ){
				...
				for(Map<String, Object> item : erpApplyAdvAmtList){
					...
					if(erpApplyAmtMap.containsKey( cdBgacct )){
						int totalAmt = CommonConvert.NullToInt( item.get( "AM_DRCR" ) ) + CommonConvert.NullToInt( erpApplyAmtMap.get( cdBgacct ) );
			...
			for(Map<String, Object> item : bgacctList){
				...
				int erpResAmt = CommonConvert.NullToInt( erpTotalAmt );
