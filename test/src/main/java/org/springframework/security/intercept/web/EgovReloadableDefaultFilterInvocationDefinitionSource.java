...
public class EgovReloadableDefaultFilterInvocationDefinitionSource extends DefaultFilterInvocationDefinitionSource implements ApplicationContextAware {
    ...
    public void reloadRequestMap() throws Exception {
        ...
        // }
        // catch ( Exception e ) {
        // logger.error( getMessageSource( ).getMessage( "error.security.runtime.error", new Object[] { "Reload RequestMap" }, Locale.getDefault( ) ), e );
        // /*
        // * 주석처리
    ...
    void addSecureUrl(String pattern, String method, ConfigAttributeDefinition attr) {
        ...
        if (logger.isDebugEnabled()) {
            logger.debug("Added URL pattern: " + pattern + "; attributes: " + attr + (method == null ? "" : " for HTTP method '" + method + "'"));
        }
    }
}
