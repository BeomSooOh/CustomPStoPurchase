...
public class SecuredObjectDAO {
	private final Log logger = LogFactory.getLog(SecuredObjectDAO.class);
	/**
	 * url 형식인 보호자원 - Role 맵핑정보를 조회하는 default 쿼리이다.
	 */
	public static final String DEF_ROLES_AND_URL_QUERY = "SELECT a.ROLE_PTTRN url, b.AUTHOR_CODE authority     " + "  FROM COMTNROLEINFO a, COMTNAUTHORROLERELATE b    " + " WHERE a.ROLE_CODE = b.ROLE_CODE                   " + "   AND a.ROLE_TY = 'url'  ORDER BY a.ROLE_SORT    ";
	public static final String DEF_ROLES_AND_URL_QUERY_KOFIA = "  SELECT '\\A' || a.url || '*\\Z' as url" + "        ,c.author_code as authority" + "        ,'0' as role_sort" + "    FROM comtnprogrmlist a" + "        ,(SELECT progrm_file_nm" + "                ,menu_no" + "            FROM comtnmenuinfo" + "           UNION ALL" + "          SELECT progrm_file_nm" + "                ,menu_no" + "            FROM comtnmenurelprogrm" + "         ) b" + "        ,comtnmenucreatdtls c" + "        ,comtnauthorinfo d" + "   WHERE a.progrm_file_nm = b.progrm_file_nm" + "     AND b.menu_no        = c.menu_no" + "     AND c.author_code    = d.author_code" + "     AND a.progrm_file_nm != 'dir'" + "     AND d.author_type_name NOT IN ('CRM' , '회계' , '감사' , '총무' , '인사' , '메신저')" + " UNION ALL " + "SELECT a.ROLE_PTTRN url, b.AUTHOR_CODE authority , a.ROLE_SORT" + "  FROM COMTNROLEINFO a, COMTNAUTHORROLERELATE b    " + " WHERE a.ROLE_CODE = b.ROLE_CODE                   "
			+ "   AND a.ROLE_TY = 'url'" + "  ORDER BY ROLE_SORT";
	/**
	 * method 형식인 보호자원 - Role 맵핑정보를 조회하는 default 쿼리이다.
	 */
	public static final String DEF_ROLES_AND_METHOD_QUERY = "SELECT a.ROLE_PTTRN \"method\", b.AUTHOR_CODE authority   " + "  FROM COMTNROLEINFO a, COMTNAUTHORROLERELATE b    " + " WHERE a.ROLE_CODE = b.ROLE_CODE                   " + "   AND a.ROLE_TY = 'method'  ORDER BY a.ROLE_SORT ";
	/**
	 * pointcut 형식인 보호자원 - Role 맵핑정보를 조회하는 default 쿼리이다.
	 */
	public static final String DEF_ROLES_AND_POINTCUT_QUERY = "SELECT a.ROLE_PTTRN pointcut, b.AUTHOR_CODE authority   " + "  FROM COMTNROLEINFO a, COMTNAUTHORROLERELATE b    " + " WHERE a.ROLE_CODE = b.ROLE_CODE                   " + "   AND a.ROLE_TY = 'pointcut'  ORDER BY a.ROLE_SORT ";
	/**
	 * 매 request 마다 best matching url 보호자원 - Role 맵핑정보를 얻기위한 default 쿼리이다. (Oracle 10g specific)
	 */
	public static final String DEF_REGEX_MATCHED_REQUEST_MAPPING_QUERY_ORACLE10G = "SELECT a.resource_pattern uri, b.authority authority   " + " FROM COMTNSECURED_RESOURCES a, COMTNSECURED_RESOURCES_ROLE b   " + " WHERE a.resource_id = b.resource_id   " + " AND a.resource_type = 'url' ";
	/*****************************************************************
	 * "SELECT a.resource_pattern uri, b.authority authority " + "FROM secured_resources a, secured_resources_role b " + "WHERE a.resource_id = b.resource_id " + "AND a.resource_id =  " + " ( SELECT resource_id FROM " + "    ( SELECT resource_id, ROW_NUMBER() OVER (ORDER BY sort_order) resource_order FROM secured_resources c " + "      WHERE REGEXP_LIKE ( :url, c.resource_pattern ) " + "      AND c.resource_type = 'url' " + "      ORDER BY c.sort_order ) " + "   WHERE resource_order = 1 ) ";
	 */
	/**
	 * Role 의 계층(Hierarchy) 관계를 조회하는 default 쿼리이다.
	 */
	public static final String DEF_HIERARCHICAL_ROLES_QUERY = "SELECT a.CHLDRN_ROLE child, a.PARNTS_ROLE parent " + "FROM COMTNROLES_HIERARCHY a LEFT JOIN COMTNROLES_HIERARCHY b on (a.CHLDRN_ROLE = b.PARNTS_ROLE) ";
	 String sqlRolesAndUrl;
	 String sqlRolesAndUrlKofia;
	 String sqlRolesAndMethod;
	 String sqlRolesAndPointcut;
	 String sqlRegexMatchedRequestMapping;
	 String sqlHierarchicalRoles;
	 String cdCompany;
	...
	public LinkedHashMap<Object, ConfigAttributeDefinition> getRolesAndResources ( String resourceType ) throws Exception {
		...
		//--  EGOV Framework 수정 끝
		/*
		 * //-- EGOV Framework 원본 시작 Iterator itr = resultList.iterator(); Map tempMap; String preResource = null; String presentResourceStr; Object presentResource; while (itr.hasNext()) { tempMap = (Map) itr.next();
		 * presentResourceStr = (String) tempMap.get(resourceType); // url 인 경우 RequestKey 형식의 key를 Map에 담아야 함 presentResource = isResourcesUrl ? new RequestKey(presentResourceStr) : (Object) presentResourceStr; List configList = new LinkedList();
		 * // 이미 requestMap 에 해당 Resource 에 대한 Role 이 // 하나 이상 맵핑되어 있었던 경우, sort_order 는 // resource(Resource) 에 대해 매겨지므로 같은 // Resource 에 대한 Role 맵핑은 연속으로 조회됨. // 해당 맵핑 Role List (SecurityConfig) 의 데이터를 // 재활용하여 새롭게 데이터 구축 if (preResource != null && presentResourceStr.equals(preResource)) { List preAuthList = (List)((ConfigAttributeDefinition) resourcesMap.get(presentResource)).getConfigAttributes(); Iterator preAuthItr = preAuthList.iterator(); while (preAuthItr.hasNext()) { SecurityConfig tempConfig = (SecurityConfig) preAuthItr.next(); configList.add(tempConfig); } }
		 * configList.add(new SecurityConfig((String) tempMap.get("authority"))); ConfigAttributeDefinition cad = new ConfigAttributeDefinition(configList);
		 * // 만약 동일한 Resource 에 대해 한개 이상의 Role 맵핑 추가인 // 경우 이전 resourceKey 에 현재 새로 계산된 Role 맵핑 // 리스트로 덮어쓰게 됨. resourcesMap.put(presentResource, cad);
		 * // 이전 resource 비교위해 저장 preResource = presentResourceStr; } //-- EGOV Framework 원본 끝
		 */
		return resourcesMap;
	}
	...
	public ConfigAttributeDefinition getRegexMatchedRequestMapping ( String url ) throws Exception {
		...
		if ( configList.size( ) > 0 ) {
			...
			logger.debug( "Request Uri : " + url + ", matched Uri : " + ((Map) resultList.get( 0 )).get( "uri" ) + ", mapping Roles : " + attributes );
		}
		return attributes;
	}
