...
public class SecuredObjectServiceImpl implements ISecuredObjectService, ApplicationContextAware, InitializingBean {
	private final Log logger = LogFactory.getLog(SecuredObjectServiceImpl.class);
	...
	public void afterPropertiesSet ( ) throws Exception {
		try {
			logger.info("ISecuredObjectService afterPropertiesSet() log.");
		}
		catch ( Exception e ) {
			if ( e instanceof Exception ) {
				...
				if ( logger.isErrorEnabled() ) {
					logger.error( messageSource.getMessage( "error.security.initialize.reason", new String[] {}, Locale.getDefault( ) ) );
					throw e;
				}
			}
		}
	...
	public LinkedHashMap getRolesAndUrl ( ) throws Exception {
		try {
			...
			logger.error( getMessageSource( ).getMessage( "error.security.runtime.error", new Object[] { "Roles and Url" }, Locale.getDefault( ) ), e );
			if ( e instanceof Exception ) {
				throw (Exception) e;
			}
			else {
	...
	public LinkedHashMap getRolesAndMethod ( ) throws Exception {
		try {
			...
			logger.error( getMessageSource( ).getMessage( "error.security.runtime.error", new Object[] { "Roles and Method" }, Locale.getDefault( ) ), e );
			if ( e instanceof Exception ) {
				throw (Exception) e;
			}
			else {
	...
	public LinkedHashMap getRolesAndPointcut ( ) throws Exception {
		try {
			...
			logger.error( getMessageSource( ).getMessage( "error.security.runtime.error", new Object[] { "Roles and Pointcut" }, Locale.getDefault( ) ), e );
			if ( e instanceof Exception ) {
				throw (Exception) e;
			}
			else {
	...
	public ConfigAttributeDefinition getMatchedRequestMapping ( String url ) throws Exception {
		try {
			...
			logger.error( getMessageSource( ).getMessage( "error.security.runtime.error", new Object[] { "MatchedRequestMapping : " + url }, Locale.getDefault( ) ), e );
			if ( e instanceof Exception ) {
				throw (Exception) e;
			}
			else {
	...
	public String getHierarchicalRoles ( ) throws Exception {
		try {
			...
			logger.error( getMessageSource( ).getMessage( "error.security.runtime.error", new Object[] { "Hierarchical Roles" }, Locale.getDefault( ) ), e );
			if ( e instanceof Exception ) {
				throw (Exception) e;
			}
			else {
