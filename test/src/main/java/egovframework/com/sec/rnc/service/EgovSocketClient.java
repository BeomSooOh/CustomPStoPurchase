...
public class EgovSocketClient {
	...
	public byte[] execute(String pmSData) {
		Socket socket = null;
		byte[] lmBRecvData = null;
		try {
			...
			sendHeader(pmSData.getBytes());
			sendData(pmSData.getBytes());
			int lmIRecvDataLength = recvHeader();
			lmBRecvData = new byte[lmIRecvDataLength];
			readAllByte(lmBRecvData, 0, lmIRecvDataLength, inStream);
		} catch (IOException e) {
	 ...
	 */
	/**
	 * 2011.10.10 cmd 라인상에서 편의제공을 위해 제공, 필요없을시 삭제하여도 무방함 public static void main(String agrs[]) { EgovSocketClient lm_oSocketClient = new EgovSocketClient(); try { /* 서비스구분코드|인증서CN|이름|주민번호
	 *
	 * 인증서 파일 에서 숫자에 해당하는 값:예) SVR1310000001_***.*** 1310000001이 인증서CN 값 이다
	 */
	/**
	 * byte[] lm_bResult1 = lm_oSocketClient.execute("12|1310000001|홍길동|6303201063614"); //System.out.println("결과:"+new String(lm_bResult1)); } catch(Exception e) { //e.printStackTrace() System.out.println(e); } }
	 */
}
