...
public class EgovTimedSocket {
	 ...
	 */
	/**
	 * 2011.10.10 cmd 라인상에서 편의제공을 위해 제공, 필요없을시 삭제하여도 무방함 public static void main(String args[]) throws Exception { try { Socket s = EgovTimedSocket.getSocketClient ("192.168.100.21", 80, 5000); s.close(); } catch (IOException ioe) { //ioe.printStackTrace(); LOG.error("Exception:  " + ioe.getClass().getName()); LOG.error("Exception  Message:  " + ioe.getMessage()); }
	 *
	 * }
	 */
	...
	static class SocketAction extends Thread {
		...
		public String getThreadInetAddr() {
			...
			else if (!EgovWebUtil.isIPAddress(threadInetAddr.getHostAddress())) { // 2011.10.25 보안점검 후속조치
