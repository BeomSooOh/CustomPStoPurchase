			...
			switch ( conVo.getErpTypeCode( ) ) {
				...
				case commonCode.ICUBE:
					...
					for(Map<String, Object> item : aaData){
						...
						double resultAmt ;
						if(("1").equals(params.get("formoptionAllocate"))){
							resultAmt = fnParseInt(item.get("erpAllocationAmt")) - fnParseInt(totalAmt);
						}
						else{
							resultAmt = fnParseInt(item.get("erpSumAmt")) - fnParseInt(totalAmt);
						}
						double amtRate = 0;
						try{
							if(fnParseInt(item.get("erpSumAmt")) == 0){
								amtRate  = 0;
							} else if (("1").equals(params.get("formoptionAllocate"))) {
								amtRate = (100 - ( resultAmt / fnParseInt(item.get("erpAllocationAmt")) * 100 ) ) ;
							} else{
				...
				for(int i=0; i< erpList.size( ) ; i++){
					...
					for(Map<String, Object> gwAData : gwList ){
						if(nullToString( erpAData.get( "erpKey" ) ).equals( nullToString(gwAData.get( "erpKey" ) )) ){
							...
							erpAData.put( "deptName", nullToString(gwAData.get( "deptName" ) ));
							erpAData.put( "empName", nullToString(gwAData.get( "empName" ) ));
						}
					}
