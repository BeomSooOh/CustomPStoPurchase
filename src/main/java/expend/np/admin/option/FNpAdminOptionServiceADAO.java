...
@Repository ( "FNpAdminOptionServiceADAO" )
public class FNpAdminOptionServiceADAO extends EgovComAbstractDAO {
	...
	public ResultVO NpGwCommonOptionUpdate ( Map<String, Object> params ) throws Exception {
		ResultVO result = new ResultVO( );
		try {
			update( "NpAdminOptionA.NpGwCommonOptionUpdate", params );
			result.setSuccess( );
		}
		catch ( Exception ex ) {
			result.setFail( "Data 질의 요청중 에러 발생", ex );
