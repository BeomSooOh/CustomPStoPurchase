...
public class BCommonProcServiceImpl implements BCommonProcService {
	...
	public ResultVO getERPResListInfoSet ( Map<String, Object> params, ConnectionVO conVo ) throws Exception {
		...
		String compSeq = nullToString(params.get("compSeq"));
		...
		if(result.getResultCode( ).equals( commonCode.SUCCESS )){
			...
			for(Map<String, Object> item : result.getAaData( )){
				...
				if(nullToString(item.get( "FILL_NB" )).equals("0") || nullToString(item.get( "FILL_NB" )).equals("")){
					aData.put( "fillNo", "" );
				} else {
					String fillNb = nullToString(item.get( "FILL_NB" ));
					String fillSq = "";
					for(int i =0; i < 5-fillNb.length() ; i++){
						fillSq += "0";
					}
					fillSq += fillNb;
					aData.put( "fillNo", nullToString(item.get( "FILL_DT" )) + "-" + fillSq);
				}
				aaData.add( aData );
