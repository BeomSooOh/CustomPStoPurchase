...
public class BCommonProcG20DAO extends EgovComAbstractDAO {
	...
	public ResultVO selectErpHincomeicList(Map<String, Object> params, ConnectionVO conVo) throws Exception {
		...
		try {
			/* parameter 검증 진행. */
			paramChecker c = ParameterCheck(params, new String[]  { "erpCompSeq", "baseDate", "trOpt2" });
			if (!c.resultCode) {
				throw new Exception(c.msg);
			}
			/* ERP DATA BASE 조회 진행 */
			List<Map<String, Object>> resultList;
			if (String.valueOf(params.get("trOpt2")).equals("2")) {
				resultList = session.selectList("g20Procedure.selectErpHincomeListBaseDate", params);
			} else {
				resultList = session.selectList("g20Procedure.selectErpHincomeList", params);
			}
			result.setAaData(resultList);
			result.setSuccess();
		} catch (Exception ex) {
