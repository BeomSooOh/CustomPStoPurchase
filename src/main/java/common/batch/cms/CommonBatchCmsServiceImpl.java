...
@Service("CommonBatchCmsService")
public class CommonBatchCmsServiceImpl implements CommonBatchCmsService {
	...
	//CMS 동기화
	  @Override
	  public int CommonSetCmsSync(Map<String,Object> params) throws InterruptedException {
	    StopWatch stopWatch = new StopWatch();
	    
	    stopWatch.start();
	    
	    logger.error("[CloudCmsRun] 새로운 batch 시작");
	      ...
	      for (Map<String, String> custInfo : arrCust) {
	          ...
	          if (CloudConstants.OPERATE_STATUS_OPERATE.equals(custInfo.get(CloudConstants.OPERATE_STATUS))) {
	              ...
	              for (CommonBatchCmsConfigVO cmsConfig : arrCmsConfig) {
					  ...
					  arrCmsData = new ArrayList<CommonBatchCmsDataVO>(); // 타 회사와 데이터 겹치지 않기 위해 CMS 데이터 초기화
...
//	                      logger.error("#[03] CMS 동기화 대상 서버가 아닙니다. [" + custInfo.toString() + "]");
	                if (BizboxAProperties.getProperty("BizboxA.CMS.Sync").equals("false")) {
	                      logger.error("#[01] CMS 동기화 대상 서버가 아닙니다. [" + custInfo.toString() + "]");
	                } else {
						  ...
						  if (mapCmsSyncParams.containsKey("erpCompSeq")) {
							  mapCmsSyncParams.remove("erpCompSeq");
						  }
	                         ...
	                         if(CommonConvert.NullToString(params.get("cmsPeriodSync")).equals("Y") && con.getErpTypeCode().equals(commonCode.ICUBE)) {
	                          ...
	                          if (con.getErpTypeCode().equals(commonCode.ERPIU)) {
	                              ...
	                              if (cmsConfig.getCustomYN().equals(commonCode.EMPTYNO)) {
	                                  ...
	                                  try {
										  // iU이면서 커스텀 프로시저가 아닌 경우에만 옵션 체크
										  if (!cmsConfig.getAllCompSyncYN().equals("Y")) {
											  // Y가 아닌 경우 파라메타에 ERP 회사코드 추가
											  mapCmsSyncParams.put("erpCompSeq", con.getErpCompSeq());
										  }
	                          ...
	                          // iCUBE 연동에 해당하는 경우
	                              ...
	                              if (cmsConfig.getCustomYN().equals(commonCode.EMPTYNO)) {
	                                  ...
	                                  try {
										  ...
										  // 통합과 Select 값이 호환되는 커스텀 프로시저면 통합과 동일한 로직 타도록 처리
										  if (cmsConfig.getCustomNewTypeYN().equals(commonCode.EMPTYYES)) {
											  cmsSyncTypeNewAll = true;
										  }
	                                  } catch (Exception e) {
	                          ...
	                          if(cmsSyncTypeNewAll && con.getErpTypeCode().equals(commonCode.ICUBE)) {
	                            ...
	                            logger.error("[CloudCmsRun] CommonCmsACardTempToCardTempBatch 시작 ");
	                            TempTime.start();
	                              ...
	                              TempTime.stop();
	                              logger.error("[CloudCmsRun] CommonCmsACardTempToCardTempBatch 종료 ");
	                              logger.error("[CloudCmsRun " + mapCmsSyncParams.get("groupSeq") + "] CommonCmsACardTempToCardTempBatch 조회 걸린 시간 : " + TempTime.getTotalTimeSeconds()  );
	      ...
	      logger.error("[CloudCmsRun] 새로운 batch 종료");
	  ...
	  public void CommonCmsACardTempToCardTempBatch(StringBuffer sb, Map<String, Object> custInfo) throws Exception {
	    ...
	    @Override
	    public List<CommonBatchCmsConfigVO> selectCms(Map<String, String> param) throws Exception {

	        List<CommonBatchCmsConfigVO> cmsConfigListVo = new ArrayList<CommonBatchCmsConfigVO>();
	        cmsConfigListVo = daoBizbox.CommonCmsConfigInfoListSelect(param);

	        return cmsConfigListVo;
	    }
}