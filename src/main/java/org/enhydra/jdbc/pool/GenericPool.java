/*
 * XAPool: Open Source XA JDBC Pool Copyright (C) 2003 Objectweb.org Initial Developer: Lutris Technologies Inc. Contact: xapool-public@lists.debian-sf.objectweb.org
 *
...
public class GenericPool {
    ...
    private Logger log;
    /**
     * checking level object 0 = no special checking 1 = just a check on an object 2 = test the object 3 = just a check on an object (for all the objects) 4 = test the object (for all the objects)
     */
    ...
    // public static final long DEFAULT_SLEEPTIME = 60000; // 5 minutes
    ...
    public GenericPool(PoolHelper helper) {
        this(helper, DEFAULT_MINSIZE, DEFAULT_MAXSIZE, DEFAULT_EXPIRATION, DEFAULT_SLEEPTIME);
    }

    public GenericPool(PoolHelper helper, int initSize) {
        this(helper, DEFAULT_MINSIZE, initSize, DEFAULT_EXPIRATION, DEFAULT_SLEEPTIME);
    }
    ...
    /*
     * Returns object from the pool or creates a connection outside synchronization to prevent hanging.
     */
    @SuppressWarnings({"rawtypes", "unchecked"})
    private Object getFromPool(String user, String password) throws Exception {
            ...
            while (e.hasMoreElements()) { // for each objects ...
                synchronized (this) {
                    if (!e.hasMoreElements()) {
                        break; // recheck for synchronization.
                    }
                    o = (GenerationObject) e.nextElement();
                    life = (Long) unlocked.get(o);
                    if (life == null) {
                        // Fix for #303462; note that this fixes the problem,
                        // but Enumeration's on Hashtable's
                        // are by definition somewhat unpredictable; a more
                        // robust fix may be in order
                ...
                if ((now - life.longValue()) > lifeTime) {
                    ...
                    log.debug("GenericPool:getFromPool check the owner of the connection");
                    if (checkOwner(o, user, password)) {
                        ...
                        if ((checkLevelObject == 0) || ((checkLevelObject == 1) && poolHelper.checkThisObject(realObject)) || ((checkLevelObject == 2) && poolHelper.testThisObject(realObject))) {
                            locked.put(o, new Long(now));
                            // put it in the locked pool
                            log.debug("GenericPool:getFromPool return an object (after verification if needed)");
                            return (o.getObj()); // return this element
                        } else { // object failed validation
                            log.debug("GenericPool:getFromPool kill an object from the pool");
                            removeUnlockedObject(o);
                        }
                    } else {
        ...
        if (create) {
            ...
            log.debug("GenericPool:getFromPool no objects available, create a new one");
            try {
                ...
                log.error("GenericPool:getFromPool Error Exception in GenericPool:getFromPool");
    ...
    public Object checkOut(String user, String password) throws Exception {
        ...
        Enumeration e;
        Object realObject;
        log.debug("GenericPool:checkOut UnlockedObjectCount=" + getUnlockedObjectCount());
        log.debug("GenericPool:checkOut LockedObjectCount=" + getLockedObjectCount());
        log.debug("GenericPool:checkOut count=" + count + " maxSize=" + maxSize);
        if (getUnlockedObjectCount() > 0) {
            // if there are objects in the unlocked pool
            if ((checkLevelObject == 3) || (checkLevelObject == 4)) { // need to
                                                                      // verify
                                                                      // all
                                                                      // the
                                                                      // objects
                ...
                while (e.hasMoreElements()) {
                    ...
                    if ((now - ((Long) unlocked.get(o)).longValue()) > lifeTime) {
                        ...
                        log.debug("GenericPool:checkOut check the owner of the connection");
                        if (checkOwner(o, user, password)) {
                            if (((checkLevelObject == 3) && !poolHelper.checkThisObject(realObject)) || ((checkLevelObject == 4) && !poolHelper.testThisObject(realObject))) {
                                log.debug("GenericPool:checkOut remove object checkLevelObject=" + checkLevelObject);
                                removeUnlockedObject(o);
                                // minimumObject(user, password);
        ...
        while ((obj == null) && (currentWait < getDeadLockMaxWait())) {
            ...
            try {
                ...
                log.error("GenericPool:checkOut ERROR Failed while waiting for an object: " + excp);
            }
    ...
    public void minimumObject(String user, String password) {
    	synchronized(this){
	        log.debug("GenericPool:minimumObject create object if there are less than minSize objects in the pool count =" + count);
	        if ((count < minSize) && (unlocked != null)) { // if pool has less than
	            ...
	            for (int i = count; i < minSize; i++) { // count have to be equal to
	                ...
	                try {
	                    ...
	                    log.error("GenericPool:minimumObject Error Exception in GenericPool:minimumObject");
	                }
	            }
	            log.debug("GenericPool:minimumObject count=" + count + " Unlocked=" + this.getUnlockedObjectCount() + " locked=" + this.getLockedObjectCount());
	...
	public void checkIn(Object o) {
		synchronized(this){
	        ...
	        for (Enumeration enum1 = locked.keys(); enum1.hasMoreElements();) { // for
	                                                                            // each
	                                                                            // object
	                                                                            // of
	            GenerationObject obj = (GenerationObject) enum1.nextElement();
	            // the locked pool
	        ...
	        if (count > maxSize) {
	            ...
	            log.info("GenericPool:checkIn more than maxSize object in the pool");
	            Enumeration enum1 = unlocked.keys(); // get the unlocked pool
	            ...
	            if (count > maxSize) {
	                log.warn("GenericPool:checkIn Be careful, the maximum size of the pool does not correspond" + " to your data. When objects will be check in, the pool " + "will decrease");
	            }
	        }
		}
    }

	private void removeUnlockedObject(GenerationObject obj) {
		synchronized(this){
			--count;
	...
	public void setMinSize(int min) throws Exception {
		synchronized(this){
	        if (min < 0) {
	            throw new Exception("GenericPool:setMinSize Minimum size of the pool can't be lesser than 0");
	        }
	        else if (min > maxSize) {
	            throw new Exception("GenericPool:setMinSize Minimum size of the pool can't be greater than the maxSize (" + maxSize + ")");
	        }
	        else {
	            this.minSize = min;
	...
	public void setMaxSize(int max) throws Exception {
		synchronized(this){
	        if (max < 0) {
	            throw new Exception("GenericPool:setMaxSize Maximum size of the pool can't be lesser than 0");
	        }
	        else if (max < minSize) {
	            throw new Exception("GenericPool:setMaxSize Maximum size of the pool can't be lesser than the minSize (" + minSize + ")");
	        }
	        else {
	            this.maxSize = max;
	            if (count > max) { // if pool has more than max element
	                log.info("GenericPool:setMaxSize pool has more than max element");
	                ...
	                for (int i = max; i < count; i++) {
	                    if (getUnlockedObjectCount() > 0) {
	                        ...
	                        GenerationObject o = (GenerationObject) enum1.nextElement();
	                        removeUnlockedObject(o);
	                    }
	                }
	                // new size is :
	                ...
	                if (count > max) {
	                    log.warn("GenericPool:setMaxSize Be careful, the maximum size of " + "the pool does not correspond to your data. When objects " + "will be check in, the pool will decrease");
	                }
	            }
	        }
		}
    ...
    public void setGeneration(int generation) {
        ...
        log.debug("GenericPool:setGeneration Be careful, it is very dangerous to change " + "the generation number, many objects could be destroyed");
    }
    ...
    protected void cleanUp() {
        ...
        long now = System.currentTimeMillis(); // current time
        synchronized (this) {
            for (Enumeration enum1 = unlocked.keys(); enum1.hasMoreElements();) {
                GenerationObject o = (GenerationObject) enum1.nextElement();
                // unlocked pool
        ...
        if (resize) {
            ...
            log.info("GenericPool:cleanUp less than minSize objects in the pool " + "min=" + minSize + " max=" + maxSize + " count=" + count);
            while (true) {
                try {
                    GenerationObject genObject = poolHelper.create();
                    synchronized (this) {
                    ...
                    e.printStackTrace();
                    log.error("eap GenericPool:cleanUp", e);
                    log.error("eap GenericPool:cleanUp Error Exception in GenericPool:cleanUp");
                    try {
            ...
            log.info("GenericPool:cleanUp done " + "min=" + minSize + " max=" + maxSize + " count=" + count);
        }
    }
    ...
    @SuppressWarnings("rawtypes")
    void expireAll() {
        log.debug("GenericPool:expireAll close all object in the unlocked and locked structures");
        for (Enumeration enum1 = unlocked.keys(); enum1.hasMoreElements();) { // for
                                                                              // each
                                                                              // object
                                                                              // of
            GenerationObject o = (GenerationObject) enum1.nextElement();
