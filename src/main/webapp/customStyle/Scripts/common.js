/**************************************************
   ...
   20190225 -  tob_box 높이 떨어짐 함수 추가 (이준혁)
   20190227 -  auto_hei 높이 함수 추가 (이준혁)
   20190429 -  text_btn_fc 폼테이블에서 텍스트 버튼 형식 자동 넓이 맞춤 함수 추가 (이준혁)
   20190809 -  openEmpProfileInfoPop 프로필팝업 추가 (이정미)
   20190813 -  공개범위 등 선택된 사용자 툴팁
   20190826 -  윈도우 팝업 크롬,ie,사파리 크기 및 위치 맞추기
**************************************************/
 ...
 $(window).ready(function () {
	setTimeout(function(){fixed_hei()},1000); // 최초자동높이
 });
   ...
   // 기록물철별문서
	...
	function auto_hei() { //자동높이조정
		$(".auto-hei").each(function(){
			var body_hei = $("body").height(); 
			var obj_hei = $(this).attr("auto-hei");
			$(this).height(body_hei - obj_hei);
		});
    }
	
	function fixed_hei() { //최초높이고정
		$(".fixed-hei").each(function(){
        var body_hei = $("body").height(); 
		var obj_hei = $(this).attr("fixed-hei");
		$(this).height(body_hei - obj_hei);
		});
    }
...
function top_box_auto(){
	
	$(".iframe_wrap.minsize .top_box").each(function(index){

		var dtdd = $(this).find("dl").children();
		var bD_wid = $(this).find('.btn_Detail').width() + 15;
		var top_wid = $(this).width() - bD_wid ;
		var dtdd_size = $(this).find("dl").children().size();
		...
		for (var i=0; i < dtdd_size ; i++ )
		{
			tot_dl_wid += $(this).find("dl").children().eq(i).not(':hidden').outerWidth(true);
			
			var orig_wid = $(this).find("DT").not(':hidden').eq(i).width();

			$(this).find("dl").children("DT").eq(i).not(':hidden').addClass("ad_"+i);
			
			if (top_wid < tot_dl_wid)
				{
					if ($(this).find("dl").children().eq(i).prop("tagName") == "DD")
					{
						$(this).find("dl").children().eq(i).addClass("on_dd");
						$(this).find("dl").children().eq(i).siblings().remove("on_dd");
						var now_dd = $(this).find(".on_dd").first().index();

							for (var n=0; n < dtdd_size ; n++)
							{
								$(this).find(".on_dd").first().nextAll().removeClass("ad_"+(n+1));			
								var num = (i-n);
								if ($(this).find(".ad_"+num).length == 0)
								{
									var fir_wid = $(this).find("DT").not(':hidden').first().width();
									$(this).find(".ad_"+num).css("width",fir_wid);
									$(this).find(".ad_"+num).css("clear","both").addClass("ar new_wid");
									$(this).find(".ad_"+num).siblings().css("clear","none");	

								} else if($(this).find(".ad_"+num).length == 1){
						...
						break;	
					} else if ($(this).find("dl").children().eq(i).prop("tagName") == "DT"){
						$(this).find("dl").children().eq(i).css("clear","both");
					}
				} else {
										
					if ($(this).find("DT").not(':hidden').eq(i).hasClass("new_wid"))
					{
						
						$(this).find("DT").not(':hidden').eq(i).css("width",orig_wid);
					}
					$(this).find("dl").children().eq(i).removeClass("on_dd");
					$(this).find("dl").children().eq(i).css("clear","none");
...
/*폼테이블 text + button 넓이설정*/
function text_btn_fc(){

	$(".iframe_wrap.minsize .com_ta").each(function(index){ 
	
			
			if ($(this).find(".PUDD-UI-inputField").next(".PUDD-UI-Button"))
			{
				
				for (i=0;i < $(this).find(".PUDD-UI-inputField").next(".PUDD-UI-Button").size() ; i++)
				{
					var th_td_wid =	$(this).find(".PUDD-UI-inputField").next(".PUDD-UI-Button").parent("td").width();
					var th_btn_wid =$(this).find(".PUDD-UI-inputField").next(".PUDD-UI-Button").width();
					$(this).find(".PUDD-UI-inputField").next(".PUDD-UI-Button").prev(".PUDD-UI-inputField").width(th_td_wid - th_btn_wid -5 );
				}
...
/*상세 포커스*/
function tb_focus_fc() {
	$(".iframe_wrap.minsize .top_box").each(function(index){ 
		$(this).find("input").focus(function(){
			$(this).parents(".top_box").addClass("active");
		});
		$(this).find("input").blur(function(){
			$(this).parents(".top_box").removeClass("active");
		});
		$(this).find(".selectField").focus(function(){
			$(this).parents(".top_box").addClass("active");
		});
		$(this).find(".selectField").blur(function(){
			$(this).parents(".top_box").removeClass("active");
		});
	});
	

	$(".iframe_wrap.minsize .SearchDetail").each(function(index){ 
		$(this).find("input").focus(function(){
			$(this).parents(".SearchDetail").addClass("active");
		});
		$(this).find("input").blur(function(){
			$(this).parents(".SearchDetail").removeClass("active");
		});
		$(this).find(".selectField").focus(function(){
			$(this).parents(".SearchDetail").addClass("active");
		});
...
$(window).scroll(function(){
	...
	if($(window).scrollTop() > 49){
		...
		$(".btn_float").css("position","fixed"); 
		$(".btn_float").css("top","41px"); 
		$(".btn_float").css("borderBottom","1px solid #dcdcdc"); 
		$(".btn_float").css("borderLeft","1px solid #1385db"); 
		$(".btn_float").css("borderRight","1px solid #1385db"); 
		
    } else {
		$(".btn_float").css("position","absolute"); 
		$(".btn_float").css("top","inherit"); 
		$(".btn_float").css("border","none"); 
...
********************************************************************************************************************/
		...
		function com_org_fc(){
			...
			$(".com_org_set .io_div .who").each(function(){
				
				$(this).mouseover(function(){
					var win_wid = $(window).width();
					var win_hei = $(window).height();
					var doc_wid = $(document).width();
					var th_left =	$(this).closest(".io_div").position().left;
					var th_top =	$(this).closest(".io_div").position().top;
					var th_wid =	$(this).closest(".io_div").width();
					var th_hei =	$(this).closest(".io_div").height();
					var tooltip_wid = $(this).nextAll(".io_tooltip").width();
					var tooltip_hei = $(this).nextAll(".io_tooltip").height();
					var y = $(this).closest(".io_div").offset().top - $(document).scrollTop();
					var x = $(this).closest(".io_div").offset().left - $(document).scrollLeft(); 

					$(this).nextAll(".io_tooltip").show();
					$(this).nextAll(".io_tooltip").addClass("animated03s fadeInUp");
					$(this).nextAll(".io_tooltip").css("top",y + 22);
					$(this).nextAll(".io_tooltip").css("left",x);

					//오른쪽으로 붙을때
					if ((th_left + tooltip_wid) > win_wid)
					{
						 $(this).nextAll(".io_tooltip").addClass("right");
						 $(this).nextAll(".io_tooltip").css("left","inherit");
						 $(this).nextAll(".io_tooltip").css("right",  win_wid - (x+th_wid) );

					} else {
						 $(this).nextAll(".io_tooltip").removeClass("right");
						 $(this).nextAll(".io_tooltip").css("right",  "inherit" );
					}

					//아래쪽으로 붙을때
					if ((th_top + tooltip_hei +40) > win_hei)
					{
						 $(this).nextAll(".io_tooltip").addClass("down");
						 $(this).nextAll(".io_tooltip").css("top","inherit");
						 $(this).nextAll(".io_tooltip").css("bottom",  win_hei - (y-th_hei+22));

					} else {
						 $(this).nextAll(".io_tooltip").removeClass("down");
						 $(this).nextAll(".io_tooltip").css("bottom",  "inherit" );
					}

				});

				$(".com_org_set .io_div").mouseleave(function(){
					$(".io_tooltip").hide();
					$(".io_tooltip").removeClass("animated03s fadeInUp fadeInDown right down");
				});
		...
		function com_io_close() { //단일 닫기
			$(".com_org_set .io_div .clo").click(function(){
		...
		function com_io_add() { //단일 추가 (개발에 맞게 변경 요망)
			com_org_fc();
		$(".com_org_set .io_div:last-child").addClass("animated03s slideInDown2");
		setTimeout( function(){$(".io_div:last-child").removeClass("animated03s slideInDown2")} , 500);

		}

		function com_io_add() { //전체 추가 (개발에 맞게 변경 요망)
			com_org_fc();
			$(".com_org_set .io_div").addClass("animated05s slideInDown2");
...
function winOpen(url,strW,strH,strT,strL) {
	...
	} else if (agent.indexOf("chrome") != -1) { //크롬등 외
	...
	if ( strT== undefined)
	{
		...
		strT = curY + (curHeight / 2) - (hei / 2);
		strL = curX + (curWidth / 2) - (wid / 2);

	}
	
	window.open(url, '_blank','width='+wid+', height='+hei+', location=no, menubar=no, toolbar=no, status=no, top='+strT+',left='+strL+'');
}
...
function openWindow2(url,  windowName, width, height, strScroll, strResize ){
	...
	pop = window.open(url, windowName, "width=" + width + ", height=" + height + ", top=100, left="+ windowX +", scrollbars="+ strScroll+", resizable="+ strResize);
	try {pop.focus(); } catch(e){}
	return pop;
}

function win_position(th) {

	
var onc =$(th).attr("onClick");
	alert(onc);
...
//윈도우 팝업 크롬,ie,사파리 크기 및 위치 맞추기 끝
//////////////////////////////////////////////////////////////////
//BASE64
...
function openEmpProfileInfoPop(compSeq, deptSeq, empSeq) {
	...
	var pop = window.open("/gw/empProfileInfo.do?" + param, "empProfileInfoPop", "width="+ width +", height="+height+", top="+ windowY +", left="+ windowX +", scrollbars=1");
	try {pop.focus(); } catch(e){}
}