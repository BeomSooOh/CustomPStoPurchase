...
$(function(){
	$(document).ready(function(){
	    ...
	    oneConSizeInit(); //오른쪽 컨텐츠영역 사이즈
	    gnbScroll();   //gnb영역 스크롤 세팅
	    lnbScroll();   //lnb영역 스크롤 세팅
	    freebScroll(); //공통 스크롤함수
	    
	    //lnb메뉴 클릭시 메뉴 첫글자로 복귀 및 this 선택
		$(".freeb_lnb .anchor").on("click",function(){
			$(".freeb_lnb .anchor").removeClass("active");
			$(this).addClass("active");
			$(".freeb_lnb").mCustomScrollbar("scrollTo","left");
		});
	});
    ...
    //원프레임 왼쪽 컨텐츠 리사이즈 이벤트
    $(".oneContents_side_wrap").resizable({
        distance: 0,
        minWidth:230,
        maxWidth:400,
        handles: 'e',
        resize: function(e, ui) {
            ...
            var move = "<div class="+"move_screen"+" style="+"position:absolute;top:0;left:0;right:0;bottom:0;background:transparent;"+"></div>"
                ui.element.width(leftContentSize);
                ui.element.find('.resizable_inner').width(leftContentSize-splitBar-1);
                divTwo = ui.element.next();
                divTwo.width(rightContentSize);
                if(!parent.find(".oneContents_sub_wrap").children().hasClass("move_screen")){
                	parent.find(".oneContents_sub_wrap").append(move);
                };
        ...
        stop: function(e, ui) {
            ...
            var splitBar = 7;
                ...
                parent.find(".oneContents_sub_wrap .move_screen").remove();
        }
    });
    ...
    //원프레임 왼쪽 컨텐츠 리사이즈바 버튼 
    $(".oneContents_side_wrap .handle-bar").on("click",function(){
        var leftNow = $(".oneContents_side_wrap").width();
        
        if(leftNow > 7){
            $(".oneContents_side_wrap").width(7);
            $(".oneContents_side_wrap .resizable_inner").width(0);
            conSizeInit();
        }else if(leftNow <= 7){
            $(".oneContents_side_wrap").width(255);
            $(".oneContents_side_wrap .resizable_inner").width(255);
            conSizeInit();
        };
    });
...
//원프레임 컨텐츠 가로 사이즈
function oneConSizeInit(){
    var $oneContents_wrap = $(".eaContract"); 				  //content wrap
    var $oneContents_side_wrap = $(".oneContents_side_wrap"); //content in left
    var contentsWidth = $oneContents_wrap.width();     		  //content wrap width size
    var contentsHeight = $oneContents_wrap.height();     	  //content wrap height size
    var leftWidth = $oneContents_side_wrap.width();           //content in left width size
    
    $(".oneContents_sub_wrap").width( contentsWidth - leftWidth - 1 );  //content in right set width
    $(".oneContents_sub_wrap").height( contentsHeight );			    //content in right set height
    $(".oneContents_sub_wrap .sub_contents").height( contentsHeight );  //sub content in right set height
};
...
function lnbScroll(){
    ...
    var lnbHei2 = $(".oneContents_side_wrap .resizable_inner").height();
    $(".freeb_lnb").height(lnbHei-51);
    $(".oneContents_side_wrap .freeb_lnb").height(lnbHei2-51);
...
function custTopReset(){
    ...
    if($(".header_sub_wrap") != null && $(".header_sub_wrap").length > 0){
		...
		$(".freeb_contents_wrap").css("top", "54px");   //customTop 를 54px  수정
	}
