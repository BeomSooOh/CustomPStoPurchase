	 ...
	 var idx = 0;	// row 복제후 id값 변경을 위해 인덱스 선언
		...
		$(document).ready(function() {	
			...
			$("#okBtn").click(function(e){ 
				...
				var chkArray = [];
				var chkDeptArray = [];
				var grid = $("#sel_user").data("kendoGrid");

				//Getting grid dataitems
				for (i = 0; i < data.length; i++) {
					var item = data[i];
					chkArray.push(item.empSeq);
					chkDeptArray.push(item.deptSeq);
				}
				
				var selected;
				selected = chkArray.join(',') + ",";	
				var deptSelected;
				deptSelected = chkDeptArray.join(',') + ",";	
				
				var authorCode = $("#authorCode").val();	

				$.ajax({
					type:"post",
					url:'<c:url value="/cmm/system/insertUserAuth.do" />',
					datatype:"json",
					data:{"authorCode":authorCode, "authorUser":selected, "authorUserDept":deptSelected},
...
<div class="pop_wrap organ_wrap" style="width:580px;">
	...
	<div class="pop_foot">
		<div class="btn_cen pt12">
			<input type="button" value="저장" id="okBtn"/>
