...
<div class="iframe_wrap" style="min-width:1000px;">
	...
	<div class="sub_contents_wrap mb0">						
			<div class="btn_div">
				...
				<div class="right_div">
					<input type="button"  id="btnResDelete" class="puddSetup"  value="결의서삭제"><!--결의서삭제--></input>
					<!-- 
...
<script type="text/javascript">
	...
	/*	[ 품의서 삭제 ] 품의서 삭제 진행
 	--------------------------------------*/
	function fnDeleteResDoc(){
		if(!$('.chkData:checked').length){
			alert('삭제할 결의서를 선택하세요.');
			return;
		}
		...
		$('.chkData:checked').each(function(i, item){
			tripResDocs +=  ',' + JSON.parse(unescape($(this).attr('data'))).tripResDocSeq;		
		});
		tripResDocs = '(' + tripResDocs.substring(1) + ')';
		console.log( 'delete target : ' + tripResDocs );

		$.ajax({
			type : 'post'
			, url : '<c:url value='/trip/admin/report/deleteTripResDoc.do' />'
			, datatype : 'json'
			, async : false
			, data : {
				tripResDocSeqs : tripResDocs
			}
	...
	function fnSetMainGrid(aaData){
		...
		$("#divGridArea").GridTable({
            ...
            , "aoDataRender": [{          // [*] 실제 데이터 표기 방법에 대하여 지정합니다.
                ...
                no: '1',
                render: function (idx, item) {
                	return '<a class="resDoc" style="cursor:pointer" title="품의문서보기">' + item.docTitle + '</a>';
                } ,
                class: 'le'
            }, {
                no: '2',
	...
	function fnSetGrid(){
		$.ajax({
			type : 'post'
			, url : '<c:url value='/trip/admin/res/selectTripResReport.do' />'
			, datatype : 'json'
			, async : false
