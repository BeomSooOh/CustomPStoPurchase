...
function fnInitClickEvent(){
	...
	Pudd( "#grid" ).on( "gridRowDblClick", function( e ) {
		...
		if( ! evntVal ) return;
		if( ! evntVal.trObj ) return;
		selectedTransPort = evntVal.trObj.rowData;

		fnCallPopupCallback(selectedTransPort);
	});
}

/* 콜백 호출 */
function fnCallPopupCallback( param ){
	var callbackFuncName = 'setTransPortCallback';
	window.opener[callbackFuncName](param);
	self.close();
}


function fnGridSetTransPortTable(){
