...
<script type="text/javascript" src='<c:url value="/js/expend/ac/ac.code.js"></c:url>'></script>
<script type="text/javascript" src='<c:url value="/js/expend/ac/ac.util.js"></c:url>'></script>
<script type="text/javascript" src='<c:url value="/js/expend/ac/ac.table.js"></c:url>'></script>
<script type="text/javascript" src='<c:url value="/js/expend/ac/ac.form.js"></c:url>'></script>
...
<script type="text/javascript">
	var template_key = "${ViewBag.template_key}" || "";
	var c_dikeycode = "${ViewBag.diKeyCode}" || "";
	var mode = "${ViewBag.mode}" || "0"; /* 1 : 결의서, 0 : 품의서 */
	var abdocu_no = "${ViewBag.abdocu_no}" || "0";

	var abdocu_no_reffer = "${ViewBag.abdocu_no_reffer}" || ""; /* 참조품의 결의서일경우 참조하는 abdocu_no */
	var focus = "${ViewBag.focus}" || "";
	var requestUrl = "${ViewBag.requestUrl}" || "";
	var docu_mode = ""; /* 1 : 결의서, 0 : 품의서 */
	...
	$ ( function ( ) {
		...
		$ ( "#btnReset" ).click ( function ( ) {
			if ( !confirm ( '<spring:message code="common.delete.msg" />' ) ) return false;/*  삭제하시겠습니까?*/
			var abdocu_no = "0";
...
<div class="pop_sign_wrap" style="width: 999px;">
		...
		<div class="psh_btnbox">
			...
			<div class="psh_right">
				<div class="btn_cen mt8">
					<input type="button" class="psh_btn" id="btnReset" value="<%=BizboxAMessage.getMessage( "TX000002960", "초기화" )%>" /> <input type="button" class="psh_btn" id="btnApprovalOpen" value="<%=BizboxAMessage.getMessage( "TX000003154", "결재작성" )%>" />
				</div>
			</div>
		</div>
	</div>
	<div class="pop_sign_con scroll_on" style="padding: 62px 16px 20px 16px;">
			...
			<table border="0" id="erpProjectInfo-table">
				...
				<tbody>
					<tr>
						...
						<td><a href="#n" class="search-Event-H"><img src="<c:url value='/Images/ico/ico_explain.png'/>" alt="" /></a> <input type="text" style="width: 85%;" id="txt_ProjectName" readonly="readonly" class="requirement" tabindex="202" /> <!--                                <input type="hidden" id="txt_PJT_FR_DT" /> --> <!--                                <input type="hidden" id="txt_PJT_TO_DT" /> --> <input type="hidden" id="txt_IT_BUSINESSLINK" /></td>
			...
			<table style="width: 966px;" id="erpTradeInfo">
				<thead>
					<tr>
						...
						<!-- 2 -->
						<th width="100"><%=BizboxAMessage.getMessage( "TX000005413", "품명" )%></th>
						<!-- 1 -->
						<th width="80"><%=BizboxAMessage.getMessage( "TX000004231", "수량" )%></th>
						<!-- 1 -->
						<th width="80"><%=BizboxAMessage.getMessage( "TX000000468", "단가" )%></th>
						<!-- 1 -->
						<th width="100" id="thTradeTrNm"><%=BizboxAMessage.getMessage( "TX000000552", "금액" )%></th>
						<!-- 1, 2, 3 , 4(지급총액)-->
						<th width="100" id="thTradeTrNm"><%=BizboxAMessage.getMessage( "TX000000516", "공급가액" )%></th>
						<!-- 1, 2, 3 , 4(실수령액)-->
						<th width="100" id="thTradeTrNm"><%=BizboxAMessage.getMessage( "TX000004701", "부가세" )%></th>
						<!-- 1, 2, 3, 4(원천징수액) -->
		...
		<table id="erpTradeInfo-tablesample" style="display: none">
			<tr class="">
				<td width="150" id="trade-td"><a href="javascript:;" class="search-Event-T"><img src="<c:url value='/Images/ico/ico_explain.png' />" alt="검색" title="검색" /></a> <input type="text" style="width: 70%" id="txt_TR_NM" class="requirement" tabindex="20001" /> <input type="hidden" class="non-requirement" id="txt_TR_FG" /> <input type="hidden" class="non-requirement" id="txt_TR_FG_NM" /> <input type="hidden" class="non-requirement" id="txt_ATTR_NM" /> <input type="hidden" class="non-requirement" id="txt_PPL_NB" /> <input type="hidden" class="non-requirement" id="txt_ADDR" /> <input type="hidden" class="non-requirement" id="txt_TRCHARGE_EMP" /> <input type="hidden" class="non-requirement" id="txt_JIRO_CD" /> <input type="hidden" class="non-requirement" id="txt_JIRO_NM" /> <input type="hidden" class="non-requirement" id="txt_REG_NB" /> <input type="hidden" class="non-requirement" id="txt_NDEP_AM" /> <input type="hidden" class="non-requirement" id="txt_INAD_AM" /> <input
					type="hidden" class="non-requirement" id="txt_INTX_AM"
				/> <input type="hidden" class="non-requirement" id="txt_RSTX_AM" /> <input type="hidden" class="non-requirement" id="txt_WD_AM" /> <input type="hidden" class="non-requirement" id="txt_ETCRVRS_YM" /> <input type="hidden" class="non-requirement" id="txt_ETCDUMMY1" /> <input type="hidden" class="non-requirement" id="txt_DATA_CD" /> <input type="hidden" class="non-requirement" id="txt_ET_YN" /> <input type="hidden" class="non-requirement" id="txt_CTR_NM" /> <input type="hidden" class="non-requirement" id="txt_CTR_CD" /></td>
	...
	<div class="pop_wrap_dir">
		...
		<div class="pop_con">
			...
			<div class="top_box" style="overflow: hidden; display: none;" id="deptEmp_Search">
				<dl class="dl2">
					<dt class="mr0">
						<input type="checkbox" name="userAllview" id="userAllview" class="k-checkbox" value="1"> <label class="k-checkbox-label radioSel" for="userAllview" style="top: 0px; margin: 0 25px 0 10px;"><%=BizboxAMessage.getMessage( "TX000009909", "모든예산회계단위" )%></label> 범위 : <input type="radio" name="B_use_YN" id="B_use_YN_2" value="2" class="k-radio"> <label class="k-radio-label" for="B_use_YN_2" style="top: 0px; margin: 0 0 0 10px; padding: 0.2em 0 0 1.5em;"><%=BizboxAMessage.getMessage( "TX000000862", "전체" )%></label> <input type="radio" name="B_use_YN" id="B_use_YN_1" value="1" class="k-radio" checked="checked"> <label class="k-radio-label" for="B_use_YN_1" style="top: 0px; margin: 0 0 0 10px; padding: 0.2em 0 0 1.5em;"><%=BizboxAMessage.getMessage( "TX000004252", "기준일" )%></label> <input type="text" name="BASIC_DT" id="BASIC_DT" value="${basic_dt }" style="width: 80px;"> <a href="javascript:;" id="user_Search"><img
							src=" <c:url value='/Images/btn/search_icon2.png'/>" alt="<%=BizboxAMessage.getMessage( "TX000000899", "조회" )%>"
						/></a>
					</dt>
				</dl>
			</div>
