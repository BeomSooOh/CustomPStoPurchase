...
<style type="text/css">
	#tblExpendCardList th, #tblExpendCardList td { border-left-width: 1px; border-right-width: 1px; }
	#tblExpendCardList th:first-child, #tblExpendCardList td:first-child { border-left-width: 0; }
	#tblExpendCardList th:last-child, #tblExpendCardList td:last-child { border-right-width: 0; }
</style>
        ...
        cTable = $("#tblExpendCardList").dataTable({
            ...
            "fnRowCallback" : function( nRow, aData, iDisplayIndex, iDisplayIndexFull ) {
                ...
                $(nRow).on('click', (function() {
					if (!clickedRow) {
						clickedRow = true;
						var checkbox = $(this).find("input[name='inp_CardChk']:checkbox");
            ...
            "columnDefs" : [ {
                ...
                "render" : function( data ) {
                    if (data != null && data != "") {
                        return '<span onclick="fnExpendCardRowSelect(data); event.stopPropagation();"><input type="checkbox" name="inp_CardChk" id="inp_CardChk' + data.syncId + '" value="' + data.syncId + '" class="k-checkbox" /><label class="k-checkbox-label bdChk" for="inp_CardChk' + data.syncId + '"></label></span>';
                    } else {
                        return "";
                    }
                }
                ...
                "render" : function( data ) {
					...
					if (data.vatStat != null && data.vatStat != ""){
						switch(data.vatStat){
							case "0" :
								result +=  '[해외]';
								break;
    ...
    function fnExpendCardInfoUpdate( param ) {
        ...
        if(expendBudgetInfo.useYN ==='Y' && isUseBudget){
	        ...
	        if ('${ViewBag.ifSystem}' == 'ERPiU') {
	        	...
	        	//준성 예산 체크할때에 확인 하기
				data.budgetInfo.seq = data.summaryInfo.budgetSeq;
    ...
    function fnExpendCardRowSelect( param ) {
		...
		for( i = 0 ; i < objViewList.length ; i++){
			...
			if(objType === 'emp' || objType === 'dept' || objType === 'pc' || objType === 'cc'){
				...
				summaryParam.budgetSeq = param.budgetSeq;
