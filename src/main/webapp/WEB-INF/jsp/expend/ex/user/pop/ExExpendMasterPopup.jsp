...
<script>
	...
	/* 변수정의 - 매입전자세금계산서 연동시 금액 편집 설정 옵션 */
	var updateEtaxAmt = false;
	...
	/* 지급요청일자 수정 시 관리항목 적용 옵션 */
	var changeExpendReqDateThenUpdateMasterMng = false;

	/* 예산체크 스킵 관련 */
	var skipBudgetCheck = "${ViewBag.skipBudgetCheck}";
...
//     	var approvalInfo = '${formInfo.form_alert}'.split('<br />').join('\r\n');
		...
		/* 지급요청일자 수정 시 관리항목 자동적용 옵션처리 */
		$.each(option, function( idx, item ) {
			if(item.option_code == '003109') {
				if (item.option_value == 'Y') {
					changeExpendReqDateThenUpdateMasterMng = true;
				}
				return false;
			}
		});
        ...
        if (expend.get('expendSeq') === '0' || expend.get('expendSeq') === '') {
			...
			if(expend.compSeq != empInfo.compSeq){
            ...
            success : function( data ) {
                ...
                if (Object.keys(expend._events).indexOf('change') < 0) {
                    expend.bind('change', function( e ) {
						if (changeExpendReqDateThenUpdateMasterMng) {
							if (e.field == "expendReqDate") {
								var reqDate = expendReqDate.element.val().replace(/-/g, "");
								if (ifSystem == "iCUBE") {
									fnUpdateMasterMng("F3", reqDate, reqDate);
								} else if (ifSystem == "ERPiU") {
									fnUpdateMasterMng(["B22", "B23"], reqDate, reqDate);
								}
							}
						}
                        fnExpendUpdate();
...
//     	fnExCloseLayPop();
    ...
    function fnExpend_Callback_Partner( param ) {
		...
		expendPartner.set('pplNo', param.obj.pplNo);
    ...
    function fnSetGridSlipBind( source ) {
        source = (source || {});
		$('#divExpendSlip').empty();
		$('#divExpendSlip').append('<table id="tblExpendSlip"></table>');
    ...
    function fnSetGridMngBind( source ) {
    	source = (source || {});
		$('#divExpendMng').empty();
		$('#divExpendMng').append('<table id="tblExpendMng"></table>');
    	...
    	$.ajax({
    		...
    		success:function(data){
            	...
            	if (opener == "1") {
					...
					if ($(".pop_sign_sub_popup").size() > 0) {
						$parent = $(".pop_sign_sub_popup");
					}
...
//     	$(".pop_wrap_dir:not(#PLP_divMainProgPop)").remove();
		...
		// 레이어 팝업 내용 전부 제거
		if ($(".pop_sign_sub_popup").size() > 0) {
			$(".pop_sign_sub_popup").empty();
		}
		// k-calendar, k-list 계속 쌓이는거 방지를 위해 제거
		$(".k-calendar-container:not(#txtExpendDate_dateview, #txtExpendReqDate_dateview)").parent(".k-animation-container").remove();
		$(".k-calendar-container:not(#txtExpendDate_dateview, #txtExpendReqDate_dateview)").remove();
		$(".k-list-container").parent(".k-animation-container").remove();
	...
	function fnUpdateMasterMng(mngCode, code, name) {
		var mngCodeList = "";
		if (typeof(mngCode) == "string") {
			mngCodeList = mngCode;
		} else {
			try {
				for (var i = 0; i < mngCode.length; i++) {
					if (mngCodeList != "") {
						mngCodeList += "|";
					}
					mngCodeList += mngCode[i];
				}
			} catch (e) {
				mngCodeList = "";
			}
		...
		$.ajax({
			type : 'post',
			url : '<c:url value="/ex/expend/master/ExMasterMngUpdate.do" />',
			datatype : 'json',
			async : false,
			data : param,
			success : function( data ) {
			...
			error : function( data ) {
				console.log("! [EX][fnUpdateMasterMng] ERROR - " + JSON.stringify(data));
			}
		});
	}
</script>
	 ...
	 <div class="pop_sign_con fixed-hei " fixed-hei="44" style="padding-top: 42px;"> 
		<div class="pop_con">
			...
			<div class="clear mt10 ExpendSlip">
				...
				<div id="divExpendSlip" class="com_ta2 mt10">
			...
			<div class="clear mt10 ExpendMng">
				...
				<div id="divExpendMng"  class="com_ta2 mt10">
