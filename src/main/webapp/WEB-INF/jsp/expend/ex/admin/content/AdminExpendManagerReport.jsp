...
// 		$('#txtReqDate').val(toD.ProcDate());
	...
	function fnAdminExpendManagerReportSend() {
		...
		if( ($('#dtExpendReqDt').val() != '') && confirm('사용자가 변경한 지급요청일자로 전송됩니다.')){
			userData[1] = true;
		}else if(($('#dtExpendReqDt').val() != '') && confirm('기존 지급요청일자로 전송합니다.')){
			userData[1] = false;
		}else if(($('#dtExpendReqDt').val() == '')){
			userData[1] = false;
		}
		fnStartSendRecurFunc(0, checkedValue, userData, checkedValue);
	}
	...
	function fnReportSendRecurence(idx, arr, userDate){
		...
		if (item.expendErpSendYn == 'Y') {
			...
			var advExpendDt = ( userDate[0] == true ) ? $('#dtExpendDt').val().replace(/-/gi, '') :  item.expendDate.replace(/-/gi, '');
			var advExpendReqDt = ( userDate[1] == true ) ? $('#dtExpendReqDt').val().replace(/-/gi, '') : ""; //item.expendReqDate.replace(/-/gi, '');
			$.ajax({
				type : "post",
				async : true,
...
<div class="sub_contents_wrap">
	...
	<div class="btn_div mt0" style="overflow:visible;">
		...
		<div class="right_div posi_re pt10 pb10">
			...
			<span> ${CL.ex_paymentDate} <!--회계일자--> ${CL.ex_allChange}<!-- 일괄변경 --></span>
			<input id="dtExpendReqDt" value="" class="dpWid datepicker" />
			<input type="button"  id="btnClearReqDt" class="puddSetup" style="background: #fff url(/exp/Images/btn/mail_btn_reload.png) no-repeat center;"data-role="button" role="button" aria-disabled="false" tabindex="0" title="${CL.ex_reset}"/>
