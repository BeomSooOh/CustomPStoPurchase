...
<!-- javascript - src -->
<script type="text/javascript" src='${pageContext.request.contextPath}/js/expend/jQuery.exp.expend.log.js'></script>
<script type="text/javascript" src='${pageContext.request.contextPath}/js/expend/jQuery.exp.expend.ajax.js'></script>
<script type="text/javascript" src='${pageContext.request.contextPath}/js/expend/jQuery.exp.expend.date.js'></script>
<script type="text/javascript" src='${pageContext.request.contextPath}/js/expend/jQuery.exp.expend.event.js'></script>
<script type="text/javascript" src='${pageContext.request.contextPath}/js/expend/jQuery.exp.expend.datatables.js'></script>
<script type="text/javascript" src='${pageContext.request.contextPath}/js/datatables/jquery.dataTables.min.js'></script>
<script type="text/javascript" src='${pageContext.request.contextPath}/js/datatables/dataTables.fixedColumns.min.js'></script>
<script type="text/javascript" src='${pageContext.request.contextPath}/js/ex/ExOption.js'></script>

<!-- javascript -->
<script>
	/* AdminYesil2Report var */
	var gisuInfo = {};
	
	/* document.ready */
	$(document).ready(function() {
    	fnGetGisuInfo();
		fnAdminYesil2ReportInit();
		fnAdminYesil2ReportEventInit();
	});
	
	/* 초기화 */
	function fnAdminYesil2ReportInit() {
		fnAdminYesil2ReportLayout();
		fnAdminYesil2ReportDatepicker();
		fnAdminYesil2ReportInitInput();
		fnAdminYesil2ReportInitButton();
		return;
	}
	
	/* 초기화 - Layout */
	function fnAdminYesil2ReportLayout() {
		$('button').kendoButton(); /* 켄도버튼정의 */
		fnAdminYesil2ComboBox(); /* 켄도콤보박스 정의 */
		...
		fnAdminYesil2CommonCode('termTypeSel', erpTermType);
		fnAdminYesil2CommonCode('acctLevelSel', '${ViewBag.params.acctLevelList}');
		fnAdminYesil2CommonCode('executeSel', '${ViewBag.params.executeList}');
		/* fnAdminYesil2CommonCode('unitSel', '${ViewBag.params.unitList}'); */
		return;
	}
	
	/* 초기화 - Datepicker */
	function fnAdminYesil2ReportDatepicker() {
		fnInitDatePicker();
		return;
	}
	
	/* 초기화 - Event */
	function fnAdminYesil2ReportEventInit() {
		fnAdminYesil2EventKey(); /* 키보드 이벤트 정의 */
	...
	/* 초기화 - Input */
	function fnAdminYesil2ReportInitInput() {
		return;
	}
	/* 초기화 - Button */
	function fnAdminYesil2ReportInitButton() {
		var url = "";
	    ...
	    $('#deptPopBtn').click(function() {
	    	url = "<c:url value='/ex/expend/admin/ExAdminYesil2DeptPop.do'/>";
	    	fnYesil2Popup(url);
	    });
	    $('#budgetGrPopBtn').click(function() {
	    	url = "<c:url value='/ex/expend/admin/ExAdminYesil2BudgetGrPop.do'/>";
	    	fnYesil2Popup(url, '640');
	    });
	    $('#budgetDeptPopBtn').click(function() {
	    	var authDept = fnGetComboBoxParams("deptSel");
	    	
	    	url = encodeURI("<c:url value='/ex/expend/admin/ExAdminYesil2BudgetDeptPop.do?authDept="+authDept+"'/>");
	    	fnYesil2Popup(url);
	    });
	    $('#bizPlanPopBtn').click(function() {
	    	url = "<c:url value='/ex/expend/admin/ExAdminYesil2BizPlanPop.do'/>";
	    ...
	    $('#budgetAcctPopBtn').click(function() {
	    	...
	    	var cdDeptPipe = fnGetComboBoxParams("deptSel");
	    	...
	    	url = encodeURI("<c:url value='/ex/expend/admin/ExAdminYesil2BudgetAcctPop.do"+param+"'/>");
	    	fnYesil2Popup(url, 640);
	    });
	...
	/* 초기화 - 콤보박스 */
	function fnAdminYesil2ComboBox(){
		...
		$(jsonData).each(function(index, value){
			dataSource.push( { 'text' : value.NM_DEPT, 'value' : value.CD_DEPT } );
		});
		
		$('#deptSel').kendoComboBox({
			"dataTextField" : "text",
            "dataValueField" : "value",
	...
	/* 공통코드 콤보박스 그리기 */
	function fnAdminYesil2CommonCode(elemID, data){
		var id = '' + elemID;
		id = id[0] == '#' ? id : '#' + id;
	...
	/* 키보드 이벤트 정의 */
    function fnAdminYesil2EventKey(){
    ...
    /*	[팝업] 팝업 호출
   	...
   	function fnYesil2Popup(url, width){
		...
		var popY = winY + (screen.height - popHeight) / 2;
   		...
   		var pop = window.open(url, "AdminYesil2Pop", "width=" + popWidth + ", height=" + popHeight + ", left=" + popX + ", top=" + popY);
    ...
    function fnClearComboBox(elemID){
   	...
   	/*	[팝업] 결의부서 CallBack
   	--------------------------------------*/
   	function fnDeptCallback(param){
   		if(param.length == 0 ){
   			fnClearComboBox("deptSel");
 	...
 	--------------------------------------*/
   	...
   	function fnAdminYesil2Vaildation(){
 	...
 	--------------------------------------*/
	function fnAdminYesil2Search(){
   		if( fnAdminYesil2Vaildation() ){
	        ...
	        $.ajax({
	            type : 'post',
	            url : '<c:url value="/ex/expend/admin/ExAdminYesil2InfoSelect.do" />',
	            datatype : 'json',
	            async : true,
	            data : param,
	            success : function( data ) {
	            	if(data.result.aaData != null){
	            		fnSetMainList(data);
	            	}
	            },
	            error : function( data ) {
	...
	function fnSetGridView( elemID, data, aoColumn, columDefs ){
		...
		var oTable = $(id).DataTable({
            ...
            "fnRowCallback" : function( nRow, aData, iDisplayIndex, iDisplayIndexFull ) {
        		...
        		if(inquiryVal == '0'){		// 조회구분 (전체)
       				...
       				$(forData[modValue]).each(function(index, element){
       					...
       					if(termVal == '1'){		// 월
       						...
       						nRow.cells[rIndex].innerHTML = viewData[modValue].toString().replace(/\B(?=(\d{3})+(?!\d))/g, ',');
       					}else{		// 분기, 반기, 년
   							if(termVal == '2'){
        		...
        		/*셀(td) 클릭 이벤트*/
        		$(nRow).off('click').on('click', 'td', function (e) {
        			var comboBox = $('#termTypeSel').data("kendoComboBox");
        			var dataFg = comboBox.value(); /* 1:월 2:분기 3:반기 4:년 */
        			if(dataFg != '4'){	// 년도 지출결의현황 X
	        			var inx = e.target.cellIndex;
	        			var budgetYmType = $(this).parents('table').find('thead tr th').eq(inx).text();
	        			fnYesil2DetailPopup( aData, budgetYmType );
        			}
        		} ); 
            },
		...
		// 셀 병합
		if (val == '0') {
			fnAdminYesil2RowSpan1();
		}
		fnAdminYesil2RowSpan2(2);
		fnAdminYesil2RowSpan2(1);
		fnAdminYesil2RowSpan2(0);
	}

	/*	[팝업] 지출결의현황 popup
	--------------------------------------*/
	function fnYesil2DetailPopup(data, budgetYmType) {
		...
		var deptCd = fnGetComboBoxParams("deptSel");
		var budgetNm = data.NM_BUDGET;
		var budgetCd = data.CD_BUDGET;
		var budgetYm = $('#from_date').val().split('-')[0];
		var acctNm = data.NM_BGACCT;
		var acctCd = data.CD_BGACCT;
		var bizplanNm = data.NM_BIZPLAN;
		var bizplanCd = data.CD_BIZPLAN;

		/* 기간구분 */
		var comboBox = $('#termTypeSel').data("kendoComboBox");
		var dataFg = comboBox.value(); /* 1:월 2:분기 3:반기 4:년 */

		var budYmType = budgetYmType;
		if (dataFg == '1') {
			budYmType = budYmType.split('-')[1] + "<%=BizboxAMessage.getMessage("TX000000436","월")%>";
		}

		var param = "deptCd=" + deptCd + "&budgetNm=" + encodeURI(encodeURIComponent(budgetNm)) + "&budgetCd="
				+ budgetCd + "&budgetYm=" + budgetYm + "&acctNm=" + encodeURI(encodeURIComponent(acctNm))
				+ "&acctCd=" + acctCd + "&bizplanNm=" + encodeURI(encodeURIComponent(bizplanNm))
				+ "&bizplanCd=" + bizplanCd + "&budYmType=" + encodeURI(encodeURIComponent(budYmType))
				+ "&dataFg=" + dataFg;

		url = "<c:url value='/ex/expend/admin/ExAdminYesil2DetailPop.do?" + param + "'/>";

		var pop = window.open(url, "AdminYesilPop", "width=" + popWidth
				+ ", height=" + popHeight + ", left=" + popX + ", top=" + popY );
	}

	/*	[뷰-그리드] 셀 병합1
	--------------------------------------*/
	function fnAdminYesil2RowSpan1() {
		$('.DTFC_LeftBodyLiner table tbody tr').each(
				function(row) {
					var thisTr = $(this);
					if (row % 4 == 0) {
	...
	/*	[뷰-그리드] 셀 병합2
	--------------------------------------*/
	function fnAdminYesil2RowSpan2(nEq) {
		var mergeItem = "";
		var mergeCount = 0;
		var mergeRowNum = 0;
	...
	<div class="sub_contents_wrap">
		...
		<div class="top_box">
			<dl>
				...
				<dd>
					<input id="from_date" value="" class="dpWid datepicker" /> ~ <input
						id="to_date" value="" class="dpWid datepicker" />
				</dd>
				<dt style="width: 80px;"><%=BizboxAMessage.getMessage("TX000009968","결의부서")%></dt>
				<dd>
					<style>.k-list-scroller {overflow:auto;}</style>
					<input id="deptSel" style="width: 120px;" class="kendoComboBox" />
					<input id="deptPopBtn" type="button" class="normal_btn" value="<%=BizboxAMessage.getMessage("TX000000265","선택")%>">
				</dd>
		...
		<div class="SearchDetail" style="display:none;">
			<dl>
				...
				<dd style="width: 200px;">
					<input type="text" id="budgetGr" style="width: 120px;" /> <input
						id="budgetGrPopBtn" type="button" class="normal_btn" value="<%=BizboxAMessage.getMessage("TX000000265","선택")%>">
				</dd>
				<dt style="width: 80px;"><%=BizboxAMessage.getMessage("TX000007201","예산단위")%></dt>
				<dd style="width: 200px;">
					<input id="budgetDeptSel" style="width: 120px;"
						class="kendoComboBox" /> <input id="budgetDeptPopBtn"
						type="button" class="normal_btn" value="<%=BizboxAMessage.getMessage("TX000000265","선택")%>">
				</dd>
				...
				<dd style="width: 200px;">
					...
					<input id="bizPlanPopBtn" type="button" class="normal_btn"
						value="<%=BizboxAMessage.getMessage("TX000000265","선택")%>">
				</dd>
			</dl>
			<dl>
				<dt style="width: 80px;"><%=BizboxAMessage.getMessage("TX000018987","계정라벨")%></dt>
				<dd style="width: 200px;">
					<input id="acctLevelSel" style="width: 120px;"
						class="kendoComboBox" />
				</dd>
				<dt style="width: 80px;"><%=BizboxAMessage.getMessage("TX000005263","예산계정")%></dt>
				<dd style="width: 200px;">
					<input id="budgetAcctSel" style="width: 120px;"
						class="kendoComboBox" /> <input id="budgetAcctPopBtn"
						type="button" class="normal_btn" value="<%=BizboxAMessage.getMessage("TX000000265","선택")%>">
				</dd>
		...
		<div>
			<div class="com_ta2 bg_lightgray viewDiv">
				<table id="Yesil2View">
					<thead>
						<tr>
							<th>${CL.ex_budgetUnitGroupNm}</th><!--예산단위그룹명-->
							<th>${CL.ex_budgetUnitNm} </th><!--예산단위명-->
							<th>${CL.ex_bussinessPlanNm} </th><!--사업계획명-->
							<th>${CL.ex_accNm} </th><!--예산계정명-->
							<th>${CL.ex_budgetDiv} </th><!--금액구분-->
						</tr>
					</thead>
					<tbody>
						<tr>
