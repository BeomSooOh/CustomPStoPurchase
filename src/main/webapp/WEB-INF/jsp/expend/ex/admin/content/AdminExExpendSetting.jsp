...
<script>
	...
	function fnConfigExpendSettingInitInput() {
		...
		if( !formInfo || formInfo.length == 0){
			alert("<%=BizboxAMessage.getMessage("TX000018738","등록된 지출결의 양식이 존재하지 않습니다. 지출결의 양식을 등록한 후 진행하여 주시기 바랍니다.")%>");
		}
		$("#selForm").empty();
	...
	function fnGetAOptionInfo() {
		var tblParam = {};
		tblParam.sCompSeq = ("${ViewBag.empInfo.compSeq}" || '');
...
// 				console.log(result);
	...
	function fnGetOptionTableData(data) {
		...
		for (var i = 0; i < data.optionItemsSet.length; i++) {
			...
			tab[item.optionCode].detailCode = (tab[item.optionCode].detailCode ? tab[item.optionCode].detailCode
					: []);
			if (!(item.optionGbn == "001" && item.optionCode == "001003" && item.useSw == "ERPiU" && item.detailCode == "E")) { // 001003 E 옵션 비활성화
	...
	function fnSetGridData(id, data) {
		...
		mainGrid = $(id).kendoGrid({
			columns : [ {
				...
				template : function(data) {
					return '<span class="" >'+ data.optionName +'</span>';
				}
			}, {
				...
				template : function(data) {
					return '<span class="" >'+ ( data.setName || data.baseName ) +'</span>';
				}
			} ],
			dataSource : dataArr,
			height : 402,
		...
		$(id).find(".k-grid-content").find("tbody").append("<tr class=\"skip\"></tr>");
...
// 					if(data.setValue === item.detailCode){
					...
					if(!data.commonCode){
						return '<input type="text" name="inp_txt" id="inp_txt'+item.detailCode+' style="width:170px; text-aling:center;" value="'+expendOption[optionGbn][data.optionCode].setValue+'"/>';
					}else{
	...
	function fnSaveOption(){
		...
		$.each(expendOption[2],function(idx,data){
			switch (data.optionCode) {
				case "003003": // 지출결의 상신 시 결의내역 최대입력 건수 설정
					if (isNaN(data.setValue)) {
						data.setName = "150";
						data.setValue = "150";
					} else {
						if (parseInt(data.setValue) > 150) {
							data.setName = "150";
							data.setValue = "150";
						} else {
							data.setName = parseInt(data.setValue).toString();
							data.setValue = parseInt(data.setValue).toString();
						}
					}
					break;
...
<div class="sub_contents_wrap">
	...
	<div class="top_box">
		<dl>
			...
			<dd>
				<select id="selForm" style="width: 200px;" class="selectmenu">
				</select>
			</dd>
		</dl>
	</div>
	...
	<div id="" class="controll_btn">
		<button id="btnSearch" class="k-button" style="display: none;">${CL.ex_search}%></button> <%-- 검색 --%>
	...
	<div class="tab_page">
		<div class="tab_style" id="tabStrip" style="background-color: white;">
			<ul class="mb20" style="background-color: white; border-top: none;">
				<li class="k-state-active">${CL.ex_functionalOption}</li><%-- 기능옵션 --%>
				<li>${CL.ex_dateOption}</li><%-- 일자옵션 --%>
				<li>${CL.ex_screenOption}</li><%-- 화면옵션 --%>
			</ul>
			<div class="tab1">
