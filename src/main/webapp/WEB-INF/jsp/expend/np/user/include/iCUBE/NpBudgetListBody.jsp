...
<div class="pop_con" style="width: 748px;">
	...
	<div class="top_box">
		...
		<dl class="next2">
			...
			<dd>
				<select id="selectStr" class="puddSetup" pudd-style="width:100px">
	...
	<div class="btn_div cl">
		...
		<div class="right_div">
			...
			<input id="btnBudgetInfoSelect" type="button" class="puddSetup" style="padding:0 5px;" value="${CL.ex_budgetsearch}" >
...
<script>
	function fnInitPopupLayout ( ) {
		...
		setTimeout(function() {
			...
			Pudd( "#selOption2" ).getPuddObject().setSelectedIndex( "" + ( viewBagParam.param.opt02 || '1') );
		}, 500);
	}
	...
	function fnSetBudgetSelectInfoInit(){
		$('#btnBudgetInfoSelect').click(function (){
			...
			param.erpGisuDate = !!param.resDate?param.resDate.replaceAll('-',''):
				param.consDate?
						param.consDate.replaceAll('-',''):
						param.frDate.replaceAll('-','');
			param.erpBudgetDivSeq = param.erpDivSeq.replace('|', '');
			param.erpMgtSeq = param.erpMgtSeq.replaceAll('|', '');
	...
	function fnGridCommonCodeTable ( data ) {
		...
		$.each ( data, function ( idx, val ) {
			...
			if( viewBagParam.param.selectedBudgetSeqs.indexOf( '|'+ val.erpBudgetSeq +'|' ) == -1  ){
				var opt = $('#selectStr option:selected').text();
				var terms = "";

				if (opt == "코드") {
					terms = val.BGT_CD.indexOf($('#txtSearchStr').val());
				} else {
					terms = JSON.stringify(val).indexOf($('#txtSearchStr').val());
				}
