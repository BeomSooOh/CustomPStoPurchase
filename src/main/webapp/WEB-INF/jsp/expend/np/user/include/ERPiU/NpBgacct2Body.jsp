...
<div class="pop_con">
	...
	<div class="top_box">
		<dl>
			<dt class="ar" style="width:65px;">${CL.ex_connAccount}  <!--연결계정--></dt>
			<dd>
				<select id="selIsConnectedBizplan" class="selectmenu" style="width:100px;">
					<option value="">${CL.ex_all}  <!--전체--></option>
					<option value="Y" selected="selected">${CL.ex_connAccount}  <!--연결계정--></option>
				</select>
			</dd>
			<dt>${CL.ex_budget}  <!--예산-->년도</dt>
			<dd>
				<select id="selBgtYear" class="selectmenu" style="width:150px;">
				</select>
			</dd>
		</dl>
		<!-- <dl class="next2"> -->
		<dl class="next2">
			...
			<dd><input type="text" autocomplete="off" id="txtSearchStr" style="width:240px;" /></dd>
	...
	<div class="com_ta2 sc_head">
		<table>
			...
			<tr>
				<th><input type="checkbox" id="chk_bgacctAllChk"/></th>
				<th>${CL.ex_code}  <!--코드--></th>
...
<script>
	...
	// 	$("#title").text("예산계정");
		...
		$('#chk_bgacctAllChk').click(function (){
			$('.chkBgacctCode').prop('checked', $(this).prop('checked'));
		});
	}
	...
	function fnSetPageParam( param ){
		/* 	예산계정 파라미터 정의 
			...
			bizplanCode : 사업계획 코드
		*/
		param.bgacctType = $("#selBgacctType").val();
		...
		if(data.length == 0){
			...
			$.each(data,function(idx, val){
				...
				if ( !( ( searchType == '' ? JSON.stringify ( val ) : ( val [ searchType ] ? val [ searchType ] : '' ) ).indexOf ( $ ( '#txtSearchStr' ).val ( ) ) < 0 )){
					...
					var tr = document.createElement('tr');
					$(tr).data('data', val);
					$(tr).append('<td> <input type="checkbox" class="chkBgacctCode" value="' + escape(JSON.stringify(val)) + '"/> </td>')
