...
<script>
		...
		let tableData = { // TODO : 백상휘 수정.
            ...
            , "aoDataRender": [ {
				...
				render: function (idx, item) {
					...
					const expHTML = '<input type="button" '+(String(item.docStatus) === '008' ? '' : 'hidden')+' class="puddSetup" id="openExpenseRegister" value="지출"/>';
					const contHTML = '<input type="button" '+(String(item.docStatus) === '008' ? '' : 'hidden')+' class="puddSetup" id="openOrderRegister" value="계약"/>';

					return expHTML + contHTML;
				}
            }]
	        ...
	        , 'fnRowCallBack' : function(row, aData){
				...
				$(row).find('#openExpenseRegister').click(function() {
					openExpenseRegister(aData);
				});

				$(row).find('#openOrderRegister').click(function() {
					openOrderRegister(aData);
		});	
	}
		}

		let columnShow = '${loginVO.groupSeq}'.toLowerCase().includes('gntp'); // 백상휘 수정.

		if (columnShow === false) {
			tableData.aoHeaderInfo.splice(6,1);
			tableData.aoDataRender.splice(6,1);
		}
	...
	function openExpenseRegister(data) { // 백상휘 수정.

		console.log('check before request', data, ${loginVo});

		$("#pummNo").val(data.consDocSeq);
		$("#approKey").val(data.docNo);
		$("#isApp").val(String(String(data.docStatus) === '008'));
		$("#docId").val(data.docNo);

		let form = $("#gntpRegister");
		form.attr('action', '/gntpPC/web/ex/payReqForm.do');
		form.submit();
	}

	function openOrderRegister(data) { // 백상휘 수정.
	    ...
	    let form = $("#gntpRegister");
    	form.attr('action', '/gntpPC/web/orderplan/ordplan/orderReqForm.do');
      	form.submit();
	}
...
<form id="gntpRegister" name="gntpRegister" method="post" action="UserNPConsReport.jsp">
    <input type="hidden" id="pummNo" name="pummNo" value=""/> <!-- Need To Set -->
    <input type="hidden" id="approKey" name="approKey" value=""/> <!-- Need To Set -->
    <input type="hidden" id="loginId" name="loginId" value="${loginVo.id}"/>
    <input type="hidden" id="deptSeq" name="deptSeq" value="${loginVo.deptid}"/>
    <input type="hidden" id="compSeq" name="compSeq" value="${loginVo.orgnztId}"/>
    <input type="hidden" id="isApp" name="isApp" value="false"/> <!-- Need To Set -->
    <input type="hidden" id="mod" name="mod" value="v"/>
    <input type="hidden" id="docId" name="docId" value=""/> <!-- Need To Set -->
    <input type="hidden" id="userSe" name="userSe" value="${loginVo.userSe}"/>
    <input type="hidden" id="contentsEnc" name="contentsEnc" value="UTF-8"/>
</form>