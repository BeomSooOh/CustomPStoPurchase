...
<script>
	...
	function fnTransferHistorySelect(){
        ...
        $.ajax({
            type : 'post',
            url : '<c:url value="/expend/np/user/NPUserCardTransHistoryList.do" />',
            datatype : 'json',
            async : false,
            data : param,
	...
	function fnSetGrid(data){
		...
		if(pageLength == 0){
			...
			$(tr).append('<td colspan="' + $("#tblTransfer colgroup col").length + '">데이터가 없습니다.</td>')
			...
			for(var i = ((currentPage * $("#selViewLength").val()) - $("#selViewLength").val()) ; i < data.length ; i ++){
				...
				$(tr).append('<td class="ce">' + data[i].receiveDate + '</td>');
				...
				if((data[i].useYn || 'Y') == 'N'){
					...
					/* 미결의 처리 */
					if(data[i].sendYn == 'Y'){
						$(tr).append('<td class="ce" style="color: blue;">결의</td>');	
					}else{
			...
			<dl>
				<dd class="ml20">
					<select class="selectmenu" id="selDateSearchType" style="width: 100px;">
						...
						<option value="1">${CL.ex_approvalDate}  <!--사용일자--></option>
					</select>
				</dd>
				<dd>
					<div class="dal_div">
