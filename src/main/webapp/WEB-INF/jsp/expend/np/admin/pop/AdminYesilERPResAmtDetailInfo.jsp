			...
			<table>
				...
				<thead>
					<tr>
						...
						<th>${CL.ex_theNumberOfBill}  <!--전표번호--></th>
					</tr>
				</thead>
			</table>
		</div>
...
<script>
	...
	function fnAdminReportExcelDown() {
		...
		excelHeader.erpMgtName= "프로젝트/부서";
		excelHeader.note= "적요";
		excelHeader.amt= "금액";
	...
	function fnSetMainTable(param) {
		...
		for (var i = 0; i < param.length; i++) {
			...
			/*if (item.fillNb == null || item.fillNb == 0){
				var fillNo = ""
			} else {
				var fillNb = item.fillNb
				var fillNo = item.fillDt + '-' + (fillNb.length >= 5 ? fillNb:new Array(5-fillNb.length+1).join('0')+fillNb);
			}*/
			var tr = document.createElement('tr');
			...
			$(tr).append('	<td>' + fnShowText( item.fillNo ) + '</td>');
