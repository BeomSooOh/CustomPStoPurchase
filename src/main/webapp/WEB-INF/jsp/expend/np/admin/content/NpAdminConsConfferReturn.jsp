...
<div class="sub_contents_wrap">
	...
	<div class="top_box">
		<dl>
			...
			<dd>
				<div class="dal_div" style="z-index:1">
	...
	<div class="btn_div cl">
		...
		<div class="right_div">							
			<div class="controll_btn p0">
				...
				<button id="btnDelete">품의서 삭제</button>
...
<script>
	...
	function fnDeleteConsDoc(){
		...
		for(var i = 0; i < chkSels.length ; i++ ){
			...
			if(item.resDocAmt > 0){
				alert('선택한 품의서 중에서 참조품의된 품의서가 있습니다.');
				return;
			} else if(item.confferBudgetReturnYN == 'Y'){
				alert('선택한 품의서 중에서 부분 반환된 품의서가 있습니다. 반환취소 후 삭제 바랍니다.');
				return;
			} else if(item.confferReturnYN == 'Y'){
				alert('선택한 품의서 중에서 반환된 품의서가 있습니다. 반환취소 후 삭제 바랍니다.');
				return;
			}
			else {
				consDocSeqs += ", '" + item.consDocSeq + "'" ;
				docSeqs += ", '" + item.docSeq + "'" ;
				if((item.tripConsDocSeq||'')!=''){
					tripConsDocSeqs +=  ", " + item.tripConsDocSeq;
				}
			}
		}
		consDocSeqs = '(' + consDocSeqs.substr(1) + ')';
		docSeqs = '(' + docSeqs.substr(1) + ')';				
		tripConsDocSeqs = '(' + tripConsDocSeqs.substr(1) + ')';
		
		if(confirm(  chkSels.length + '건의 품의서를 삭제하겠습니까?\n삭제된 품의서는 복구가 불가능 합니다.')){
			$.ajax({
				async : false,
				type : "post",
				data : {
	            	consDocSeqs : consDocSeqs
	            	, docSeqs : docSeqs
	            	, tripConsDocSeqs : tripConsDocSeqs
	            },
				url : "<c:url value='/expend/np/admin/NpAdminConsReportDelete.do' />",
				datatype : "json",
	...
	* 	actionFlag = [true, false]
	...
	function fnSetConfferStatus( actionFlag ){
		...
		$.ajax({
			...
			type : "post",
			data : param,
			url : "<c:url value='/expend/np/admin/NpAdminConsConfferStatusUpdate.do' />",
			datatype : "json",
			success : function(result) {
				if(result.result.resultCode != 'SUCCESS'){
					alert('서버 오류 발생');
	...
	* 	actionFlag = [true, false]
	...
	function fnSetConfferBudgetStatus( actionFlag, budgetSeq ){
		...
		$.ajax({
			...
			type : "post",
			data : param,
			url : "<c:url value='/expend/np/admin/NpAdminConsConfferBudgetStatusUpdate.do' />",
			datatype : "json",
			success : function(result) {
				if(result.result.resultCode != 'SUCCESS'){
					alert('서버 오류 발생');
	...
	function fnSelectConsList(){
		var param = fnGetConslistParam();
		
		$.ajax({
			async : true,
			type : "post",
			data : param,
			url : "<c:url value='/expend/np/admin/NpAdminConsReportSelect.do' />",
			datatype : "json",
			success : function(result) {
				if(result.result.resultCode != 'SUCCESS'){
					alert('서버 오류 발생');
	...
	--------------------------------------------*/
	function fnSetConsGrid2(aaData){
		...
		$("#divGridArea").GridTable({
            ...
            , "aoDataRender": [{          // [*] 실제 데이터 표기 방법에 대하여 지정합니다.
                ...
                render: function (idx, item) {                        	                     	
                	return '<input type="checkbox" class="chkSel selData" id="chk_'+item.consDocSeq+'" value="'+item.consDocSeq+'" useYn="' + (item.resDocAmt>0?'Y':'N')  +'"/>';
                }
            }, {
                no: '1',
                render: function (idx, item) {                        	                     	
 	...
 	function fnSetConsGrid(){
		...
		if(aaData.length == 0){
			...
			$.each(aaData,function(idx, val){
				...
				$(td).text(val.docNo);
				$(tr).append(td);
				$(tr).append( '	<td>'+ val.docTitle + '</td>' );
				$(tr).append( '	<td>'+ fnGetDateFormat(val.docDate) + '</td>' );
				$(tr).append( '	<td>'+ fnGetCurrenyCode(val.consDocAmt, '-') + '</td>');
				
				/* 지출금액 결의리스트 (참조 결의서) */
				var td = document.createElement('td');
				val.colGbn = 'resDocPop';
				var data2 = JSON.parse(JSON.stringify(val));
				$(td).html('<a class="text_blue" style="text-decoration:underline">' + fnGetCurrenyCode(val.resDocAmt) + '</a>');
				$(td).data('data', data2);
				$(tr).append(td);
				
				$(tr).append( '	<td>'+ fnGetCurrenyCode(val.balanceAmt, '-') + '</td>');
	...
	function fnShowResDocList(consDocSeq){
		var param = {
			...
			$.ajax({
				...
				type : "post",
				data : param,
				url : "<c:url value='/expend/np/admin/NpAdminConsConfferResList.do' />",
				datatype : "json",
				success : function(result) {
					if(result.result.resultCode != 'SUCCESS'){
						alert('서버 오류 발생');
	...
	function fnDrawConfferResTable(aaData){
		...
		$.each(aaData,function(idx, val){
			...
			$(tr).append( '	<td>'+ fnGetCurrenyCode(val.resDocAmt, '-') + '</td>');
	...
	function fnShowBudgetList(consDocSeq){
		...
		$.ajax({
			...
			type : "post",
			data : param,
			url : "<c:url value='/expend/np/admin/NpAdminConsBudgetList.do' />",
			datatype : "json",
			success : function(result) {
				if(result.result.resultCode != 'SUCCESS'){
					alert('서버 오류 발생');
	...
	function fnDraowConsBudgetIlstTable(data){
		...
		$('#lblConsBudgetCnt').html(listData.length);
		...
		if(erpType == 'ERPiU'){
			...
			if(listData){
				...
				for(var i =0; i < listData.length; i++){
					...
					listTr += '	<td class="onlyCube">' + item.erpBudget1Name + '</td>';
					listTr += '	<td class="onlyCube">' + item.erpBudget2Name + '</td>';
					listTr += '	<td class="onlyCube">' + item.erpBudget3Name + '</td>';
					listTr += '	<td class="onlyCube">' + item.erpBudgetName + '</td>';
					listTr += '	<td>' + (item.erpDivName || item.erpPcName) + '</td>';
					listTr += '	<td class="ri">' + fnGetCurrencyCode(item.budgetAmt) + '</td>';
					listTr += '	<td class="ri">' + fnGetCurrencyCode(item.resBudgetAmt) + '</td>';
					listTr += '	<td class="ri">' + fnGetCurrencyCode(item.balanceAmt) + '</td>';
...
<form id="excelDownload" name="excel" method="post">
	...
	<input type="hidden" name="docStatus" value="'008', '009', '90'" id=""> 
</form>
			...
			<table>
				...
				<tbody id="confferResTbl">
					<tr>
						...
						<td class="le">2018년 1월 회계팀 ${CL.ex_resDoc} <!--결의서--></td>
						<td>2017-12-20</td>
                           <td>uc개발2팀</td>
						<td>홍길동</td>
                           <td>더존비즈온</td>
                           <td>${CL.ex_temporarySave} <!--임시저장--></td>
...
<div id="consBudgetListPop" class="pop_wrap_dir consBudgetListPop" style="width:1250px; top:50%; left:50%; transform:translate(-50%,-50%); display:none;">
	<div class="pop_head">
		<h1>${CL.ex_budgetListDetail} <!--예산내역 상세--></h1>
			...
			<table>
				...
				<thead>
					<tr>
						...
						<th class="onlyCube">${CL.ex_bgt1Name} <!--관--></th>
						<th class="onlyCube">${CL.ex_bgt2Name} <!--항--></th>
						<th class="onlyCube">${CL.ex_bgt3Name} <!--목--></th>
