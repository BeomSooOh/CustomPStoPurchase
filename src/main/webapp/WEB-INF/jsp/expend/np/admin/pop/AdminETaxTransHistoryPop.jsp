...
<script>
	...
	function fnTransferHistorySelect() {
		...
		$.ajax({
			type : 'post',
			url : '<c:url value="/expend/np/admin/NPAdminETaxTransHistoryList.do" />',
			datatype : 'json',
			async : false,
			data : param,
	...
	function fnSetGrid(data) {
		...
		if (pageLength == 0) {
			...
			for (var i = ((currentPage * $("#selViewLength").val()) - $("#selViewLength").val()); i < data.length; i++) {
				...
				/* 구분 */
				$(tr).append('<td class="ce">' + transfer + data[i].transferName + '</td>');
				/* 작성일자 */
				...
				if ((data[i].useYn || 'Y') === 'N') {
					...
					if ((data[i].sendYn || 'N') == 'Y') {
						$(tr).append('<td class="ce">${CL.ex_res}</td>');
					} else {
