...
<script>
	...
	function fnETaxTrans() {
		...
		if (chkSels.length > 0) {
			...
			$.each(chkSels, function(idx, item) {
				if (item.sendYn == 'N' && item.useYn == 'Y') {
					eTaxTransList.push(item);
				}
			});

			if (eTaxTransList.length > 0) {
				var confirmMsg = '이관이 완료되면 받는 사람의 사용자화면에서 정보가 조회됩니다. 계속 진행하시겠습니까?';
				if (confirm(confirmMsg)) {
					/* 공통팝업 오픈 */
	...
	function fnFilterdList(aaData){
		...
		for(var i = 0; i < aaData.length; i++){
			...
			/* 결의자 체크 */
			if( (item.createEmpName || '') .indexOf(approEmpName) == -1){
				continue;
			}
	...
	function fnTableReOrder(aaData, orderType, $th){
		...
		aaData.sort(function(a, b) {
			...
			} else if(_orderType == 'createEmpName'){
	...
	function fnEmptyInitTable() {
		...
		$table.append('<tr><th><input type="checkbox"></th><th>작성일자</th><th>공급자</th><th>사업자등록번호</th><th>관리번호</th><th>금액</th><th>공급가액</th><th>부가세</th><th>결의상태</th></tr>');
...
<input type="hidden" id="hidSelectedETaxInfo" value=""> <!-- body -->
	<div class="sub_contents_wrap">
		...
		<div class="top_box">
			<dl>
				<dt>${CL.ex_writeDate}</dt> <!-- 작성일자 -->
				<dd>
					<div class="dal_div">
						<input type="text" autocomplete="off" id="txtFromDate" value="" class="w113 enter" /> <a href="#n" id="btnFromDate" class="button_dal"></a>
					</div>
					~
					<div class="dal_div">
						<input type="text" autocomplete="off" id="txtToDate" value="" class="w113 enter" /> <a href="#n" id="btnToDate" class="button_dal"></a>
					</div>
				</dd>
				<dt>${CL.ex_supplyer}  <!--공급자--></dt>
				<dd>
					<input type="text" autocomplete="off" id="txtPartnerName" style="width: 186px;" placeholder="${CL.ex_supplier} ${CL.ex_Please}" />
				</dd>
		...
		<div class="SearchDetail mb10">
			<dl>
				...
				<dt style="width: 70px;">${CL.ex_manageNum}  <!--관리번호--></dt>
				<dd>
					<input type="text" autocomplete="off" id="txtIssNo" style="width: 150px;" placeholder="${CL.ex_manageNum} ${CL.ex_Please}" /><!-- 관리번호 -->
				</dd>
