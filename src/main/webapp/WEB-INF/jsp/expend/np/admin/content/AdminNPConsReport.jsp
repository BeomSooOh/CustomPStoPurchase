        ...
        <dl class="next2">
            ...
            <!-- <dd>
                <select class="selectmenu" id="selConfferReturnYN" style="width:100px;">
                    ...
                    <option value="">${CL.ex_yes}</option>
                    <option value="">${CL.ex_no} </option>
                </select>
            </dd> -->
...
<script>
	...
	function fnSetConsGrid2(aaData){
		...
		$("#divGridArea").GridTable({
            ...
            , "aoDataRender": [{          // [*] 실제 데이터 표기 방법에 대하여 지정합니다.
            	...
            	no: '6',
                render: function (idx, item) {                        	                     	
                	return fnGetCurrencyCode(item.consDocAmt);
                } ,
                class: 'ri'
            }, {
            	no: '5',
	...
	---------------------------------------- */
    function fnSetConsGrid(data) {
        $('#tblConsGrid .consGridTr').remove();
        var appendHTMLCode = '';
        console.log(data);
        ...
        for (var i = 0; i < data.length; i++) {
            ...
            console.log(item);
            var trCode = '';
            trCode += '	<tr class="consGridTr">';
            trCode += '		<td onclick=fnAprroDocOpen("' + item.docSeq + '") style="cursor:pointer;">' + (item.docNo || '') + '</td>';
            trCode += '		<td onclick=fnAprroDocOpen("' + item.docSeq + '") style="cursor:pointer;" class="le">' + (item.docTitle || '') + '</td>';
            trCode += '		<td>' + (item.expendDate || '') + '</td>';
            trCode += '		<td>' + (item.deptName || '') + '</td>';
            trCode += '		<td>' + (item.empName || '') + '</td>';
            trCode += '		<td>' + fnGetDocStatusLabel(item.docStatus) + '</td>';
            trCode += '		<td class="ri">' + fnGetCurrencyCode(item.consDocAmt) + '</td>';
            trCode += '		<td class=""><span class="ico-blank" onclick="openPopApprovalLine(' + item.docSeq + ')"></span></td>';
            trCode += '	</tr>';
            appendHTMLCode += trCode;
        }
        $('#tblConsGrid').append(appendHTMLCode);
    }
    ...
    function openPopApprovalLine(docId) {
        var param = "diKeyCode=" + docId + "&doc_auth=1";
