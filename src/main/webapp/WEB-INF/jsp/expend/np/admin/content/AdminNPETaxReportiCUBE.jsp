...
<script>
	...
	ViewBag = {
			empInfo={
					...
					,"eaType":"ea"
					,"empSeq":"707010012078"
					,"compSeq":"707010010252"
					,"userSe":"USER"
					,"deptSeq":"707010012077"
					,"bizSeq":"707010010252"
					}
			, conVo={
					"g20YN":"Y"
	...
	function fnETaxTransHistory() {
		/* 팝업 호출 준비 */
		var url = "<c:url value='/expend/np/admin/AdminETaxTransHistoryPop.do'/>";
		var height = 480;
	...
	function fnETaxSearch() {
		...
		$.ajax({
			type : 'post',
			url : "<c:url value='/expend/np/admin/NpGetETaxList.do' />", 
			datatype : 'json',
			async : true,
			data : ajaxParam,
			ajaxParam : ajaxParam,
	...
	function fnFilterdList(aaData){
		...
		for(var i = 0; i < aaData.length; i++){
			...
			/* 결의상태 체크 */
			if( (item.sendYn || 'N') .indexOf(approYN) == -1){
				continue;
			}
	...
	function fnTableReOrder(aaData, orderType, $th){
		...
		aaData.sort(function(a, b) {
			...
			} else if(_orderType == 'reqAmtCon'){
				if( a.reqAmtCon < b.reqAmtCon ){
					return _orderBy;
				}else {
					return _orderBy * -1;
				}
			} else if(_orderType == 'stdAmtCon'){
				if( a.stdAmtCon < b.stdAmtCon ){
					return _orderBy;
				}else {
					return _orderBy * -1;
				}
			} else if(_orderType == 'vatAmtCon'){
				if( a.vatAmtCon < b.vatAmtCon ){
					return _orderBy;
				}else {
					return _orderBy * -1;
				}
			} else if(_orderType == 'eTaxTy'){
				if( a.eTaxTy < b.eTaxTy ){
					return _orderBy;
				}else {
					return _orderBy * -1;
				}
			} else if(_orderType == 'approEmpName'){
	...
	/* ====================================================================================================================================================== */
		...
		$("#divGridArea").GridTable({
	        ...
	        , "aoHeaderInfo": [{                  // [*] 테이블 헤더 정보입니다.
	            ...
	            no: '8'
			        , renderValue: '${CL.ex_classification}'
		            , colgroup: '18', 
					class : 'orderBy', 
					value : 'eTaxTy'
		    }, {
            ...
            , "aoDataRender": [{          // [*] 실제 데이터 표기 방법에 대하여 지정합니다.
                ...
                render: function (idx, item) {          
                	...
                	return '<input type="checkbox" name="chkETax" onclick="Common.Util.CheckboxStat();" class="chk_row" ' + isDisabled + '/>';
                }
            }, {
                no: '1',
                render: function (idx, item) {                        	                     	
                ...
                no: '5',
                render: function (idx, item) {                        	                     	
                	return Common.Format.Amt(item.reqAmtCon);
                } ,
                class: 'ri colorIf'
            }, {
                no: '6',
                render: function (idx, item) {                        	                     	
                	return Common.Format.Amt(item.stdAmtCon);
                } ,
                class: 'ri colorIf'
            }, {
                no: '7',
                render: function (idx, item) {                        	                     	
                	return Common.Format.Amt(item.vatAmtCon);
                } ,
            	class : 'ri colorIf'
            }, {
                no: '8',
                ...
                render: function (idx, item) {
        			...
        			/* 상신 : 결의(홍길동) */
        			if (item.useYn === 'N') {
        				return '<span  style="color: red">${CL.ex_notUse}</span>&nbsp;(' + (item.notUseEmpName || '-') + ')';
        			} else {
        				if (item.sendYn === 'Y') {
        					return '${CL.ex_res} (' +  (item.approEmpName || '-') + ')';
        				} else {
        					return '${CL.ex_noRes}';
        				}
        			}
	...
	function fnEmptyInitTable() {
		...
		$(td).attr('colspna', $('.com_ta2 table tr:eq(0) th').length);
	...
	function fnETaxExcel() {
		...
		/* submit */
		var url = "<c:url value='/expend/np/admin/NpGetETaxListExcel.do' />";
...
<form id="excelDownload" name="excel" method="post">
	...
	<input type="hidden" name="issDateFrom" value="" id="issDateFrom" />
	<input type="hidden" name="issDateTo" value="" id="issDateTo" />
	<input type="hidden" name="partnerName" value="" id="partnerName" />
	<input type="hidden" name="eTaxStat" value="" id="eTaxStat" />
	<input type="hidden" name="partnerRegNo" value="" id="partnerRegNo" />
	<input type="hidden" name="eTaxIssNo" value="" id="eTaxIssNo" value" />
	<input type="hidden" name="approvalEmpName" value="" id="approvalEmpName" />
</form>
<!-- 이관 관련 데이터 -->
...
<!-- 			</table> -->
		...
		<!-- 테이블 -->
		<div class="com_ta2">
			<table id="tblUserETaxReport"></table>
		</div>
	...
	<form id="frmPop2" name="frmPop2">
		...
		<input type="hidden" name="callback" width="500" value="fnETaxTransCallback" />
