...
<script>
	/* 공통코드 url 설정 */
	var url = "<c:url value='/expend/np/user/NpCommonCodePop.do'/>";
	var popupWidth = 600;
	var popupHeight = 400;
	var basicPopupType = "";
	/* 공통 팝업 호출 */
		...
		$.ajax({
	        ...
	        success : function( data ) {
	        	...
	        	if(!!(JSON.parse(params.param).selectedBudgetSeqs)){
	        		var newAaData = [];
					var selectedBudgetSeqs = JSON.parse(params.param).selectedBudgetSeqs;

					if(Option.Common.ERPiU()){
						/* iu인 경우 예산단위-(사업계획)-예산계정 */
						for(var i = 0; i < data.result.aaData.length; i++ ){
							var item = data.result.aaData[i];
							var selectedBgacctCd = '|' + (item.erpBudgetSeq || item.CD_BUDGET) + '|' + (item.erpBizPlanSeq || item.CD_BIZPLAN || '***') + '|' + (item.erpBgacctSeq || item.CD_BGACCT) + '|' ;
							if( (selectedBudgetSeqs.indexOf(selectedBgacctCd) == -1) ){
								newAaData.push(item);
							}
						}
					} else {
						...
						for(var i = 0; i < data.result.aaData.length; i++ ){
							var item = data.result.aaData[i];
							if( (selectedBudgetSeqs.indexOf('|' + (item.erpBudgetSeq || item.BGT_CD) + '|') == -1) ){
								newAaData.push(item);
							}
						}
					}
	...
	function fnDirectSearchCallback( params, data ){
		...
		switch(params.code){
			...
			case "bgacct2":
			case "dept2":
				var returnData = [escape(JSON.stringify(data.result.aaData[0]))];
				break;
			default :
