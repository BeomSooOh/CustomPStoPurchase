...
(function ($) {
	...
	$.fn.PuddCombo = function( _opt ){
		...
		$.each( $( this ), function(index, item){
			...
			PuddManager.makeComponent( $( this ), puddCombo );
		});
	};
	...
	ComponentCommander.combo.prototype.setSelected = function( _json ){
		...
		if( json.index != undefined ){
			$cloneList.find( 'li:eq(' + json.index + ')' ).click( );			
		}else if( json.text != undefined ){
			$cloneList.find( 'li:contains('+ json.text +')' ).click( );			
		}
	};
	
	ComponentCommander.combo.prototype.setAddItem = function( _json ){
		if( _json == undefined ){  console.error( '인자 값이 없습니다.' ); return false; }
		var json = CommonUtil.castToJSONObject( _json );
		var $selector = CommonUtil.extractSelector( _json );
		var $cloneList = $( "#" + $selector.attr( 'id' ) + '_showDiv' );
		...
		if( json.text == undefined || json.value == undefined ){
			console.error( '추가할 항목의 정보가 부족합니다.' );
			return false;
		}
		//생성
		var OPTION = "<option value="+ json.value +">"+ json.text + "</option>";
	...
	/* 비활성화 */
	ComponentCommander.combo.prototype.setDisabled = function( _json ){
		if( _json == undefined ){  console.error( '인자 값이 없습니다.' ); return false; }
		var json = CommonUtil.castToJSONObject( _json );
		var $selector = CommonUtil.extractSelector( _json );
	...
	/* 활성화 */
	ComponentCommander.combo.prototype.setEnabled = function( _json ){
		if( _json == undefined ){  console.error( '인자 값이 없습니다.' ); return false; }
		var json = CommonUtil.castToJSONObject( _json );
		var $selector = CommonUtil.extractSelector( _json );
	...
	PuddCombo.prototype = {
		...
		init : function( ){
			...
			this.inOnChange = json.onchange || null;
		...
		setElementAssembly : function( ){					
			...
			switch( type ){
				case 'BASIC' :
					...
					puddAppendElement.appendOPTION( this.type, this.$SELECT, this.option.dataSource, this.option.index );
					...
					puddAppendElement.appendLI( this.type, this.$UL, this.option.dataSource, this.option.index );
		...
		setElementStyle : function( ){
			puddComboStyle.pushDivStyle( this.type, this.$DIV, this.option.style || { } , this.outStyle );
			puddComboStyle.pushShowDivStyle( this.$SHOW_DIV, this.option.style || { } , this.inStyle );
		},
		...
		setElementAttribute : function( ){
			...
			puddComboAttribute.pushSelectAttribute( this.$SELECT, this.id, this.option, this.inOnChange );
			if( this.type ==='BASIC' ){
	...
	var puddAppendElement = {
			appendOPTION : function( type, $SELECT, json, index ){
			...
			appendLI : function( type, $UL, json, index ){
				var arrJson = ( json == undefined ? [ ] : CommonUtil.castToJSONObject( json ) );
				var selectIndex = -1;
	 ...
	 */
	var puddComboStyle = {		
			pushDivStyle : function( type, $element, styleOption, outStyle ){
				...
				if( type === 'MULTI' && styleOption.height != undefined ){
			...
			pushShowDivStyle : function( $element, styleOption, inStyle ){
				$element.css( 'top', '100%' );;
	...
	var puddComboAttribute = {
			...
			pushSelectAttribute : function( $element, id, option, onChange ){
				//id 생성
				if( id != undefined && id !== ''){
					$element.attr( 'id', id );
				}
...
//			var selectChk = This.offset( ).top;
//			var ScHeight = $(window).scrollTop( );
//			var listChk = Number(150);
//			var bottomHeight = ( (bodyChk - selectChk - parentHeight) + ScHeight );
//			if(bottomHeight < listChk){
//				$SHOW_DIV.css({"top":"inherit","bottom":"100%"});	
//			}else{
//				$SHOW_DIV.css({"top":"100%","bottom":"inherit"});
//			};
			...
			var elemHeight2 = (isNaN(Number(This.height())) ? 0 : Number(This.height())) + 4;
			...
			if(bottomHeight < listHeight){
				...
				//아래로
				if($SHOW_DIV.hasClass("PUDD-UI-selectBox")){					
...
//			};
			...
			if(bottomHeight < listHeight){
				...
				//아래로
				if($SHOW_DIV.parent( ).hasClass("PUDD-UI-selectBox")){					
					$SHOW_DIV.css({"width":parentWidth-2 + "px","top":((ThisY + elemHeight)-ScHeight + "px"),"left":ThisL + "px","bottom":"inherit"});	
