...
(function ($) {
	...
	$.fn.PuddText = function( opt ){
		...
		$.each( $( this ), function(index, item){
			...
			PuddManager.makeComponent( $( this ), puddInput );
		});
	};
	...
	ComponentCommander.input.prototype.setStateChange = function( _json ){ 
		...
		$selector.after( CommonUtil.searchSVGObject( item.toUpperCase( ) ) )
	...
	/* 비활성화 */
	ComponentCommander.input.prototype.setDisabled = function( _json ){
		if( _json == undefined ){  console.error( '인자 값이 없습니다.' ); return false; }
		var json = CommonUtil.castToJSONObject( _json );
		var $selector = CommonUtil.extractSelector( _json );
	...
	/* 활성화 */
	ComponentCommander.input.prototype.setEnabled = function( _json ){
		if( _json == undefined ){  console.error( '인자 값이 없습니다.' ); return false; }
		var json = CommonUtil.castToJSONObject( _json );
		var $selector = CommonUtil.extractSelector( _json );
	...
	PuddInput.prototype = {
		...
		setElementStyle : function( ){
			puddInputStyle.pushDivStyle( this.$DIV, this.option.style || { } , this.outStyle );
			puddInputStyle.pushInputStyle( this.$INPUT, this.option.style || { }, this.inStyle );
		},
		...
		setElementAttribute : function( ){
			...
			if( this.INPUT != null){
				puddInputAttribute.pushInputAttribute( this.$INPUT, this.option, this.id );
			}
	...
	var puddInputStyle = {		
			...
			pushInputStyle : function( $element, styleJson, inStyle ){
				...
				//옵션 스타일 지정
				if( styleJson.align != undefined ){
					$element.css( 'text-align', styleJson.align );
				}
				if( inStyle != null ){
	...
	InputEvent.prototype.autoComplete = function( event ){
		...
		function selectPosition( obj ){
			...
			if(bottomHeight < listHeight){
				//위로	
				This.parent().find(".autoComplete").css({"width":parentWidth-2 + "px","bottom":(listHeight + 12) + "px","left":ThisL + "px","top":"inherit"});
			}else{
				//아래로
