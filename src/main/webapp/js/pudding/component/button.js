...
(function ($) {
	...
	$.fn.PuddButton = function( _opt ){	
		$.each( $( this ), function(index, item){
			...
			PuddManager.makeComponent( $( this ), puddButton );
		});
	...
	/* 비활성화 */
	ComponentCommander.button.prototype.setDisabled = function( _json ){
	...
	/* 활성화 */
	ComponentCommander.button.prototype.setEnabled = function( _json ){ 
		if( _json == undefined ){  console.error( '인자 값이 없습니다.' ); return false; }
		var json = CommonUtil.castToJSONObject( _json );
		var $selector = CommonUtil.extractSelector( _json );
	...
	PuddButton.prototype = {
		...
		setElementStyle : function( ){
			puddButtonStyle.pushDivStyle( this.$DIV, this.option.style || { } , this.outStyle );
			puddButtonStyle.pushInputStyle( this.$INPUT, this.option.style || { }, this.inStyle, this.type );
		},
		...
		setElementAttribute : function( ){
			...
			if( this.INPUT != null){
				puddButtonAttribute.pushInputAttribute( this.$INPUT, this.option, this.id, this.type, this.inName, this.inOnClick );
			}
		},
	...
	var puddButtonStyle = {		
			pushDivStyle : function( $element, styleOption, outStyle ){
				...
				if( styleOption.cursor != undefined ){				
					$element.css( 'cursor:', styleOption.cursor );
				}
				if( styleOption.margin != undefined ){				
					$element.css( 'margin', styleOption.margin );
				}				
				if( outStyle != null ){
			...
			pushInputStyle : function( $element, styleJson, inStyle, type ){
	 			styleJson = styleJson == undefined ? { } : styleJson; 
				//옵션 스타일 지정
				if( styleJson[ 'font-size' ] != undefined ){
					$element.css( 'font-size', CommonUtil.castUnit( styleJson[ 'font-size' ] ) );
				}
				
				if( styleJson[ 'border-radius' ] != undefined ){
					$element.css( 'border-radius', CommonUtil.castUnit( styleJson[ 'border-radius' ] ) );
				}
				
				if( styleJson.padding != undefined ){
					$element.css( 'padding', styleJson.padding  );
				}
			
				if( type == 'BASIC' || type == 'IMAGE' || type == 'IMAGETEXT'  ){
					if( styleJson.background != undefined ){
						$element.css( 'background', styleJson.background  );
					}
				}
				
				if( type == 'IMAGE' || type == 'IMAGETEXT'  ){
					if( styleJson[ 'background-position' ] != undefined ){
						$element.css( 'background-position', styleJson[ 'background-position' ]  );
					}
				}
				
				if( type == 'IMAGE'|| type == 'IMAGETEXT'  ){
					if( styleJson[ 'background-reapeat' ] != undefined ){
						$element.css( 'background-reapeat', styleJson[ 'background-reapeat' ]  );
					}
				}
				
				if( type == 'IMAGE' || type == 'IMAGETEXT'  ){
					if( styleJson[ 'background-size' ] != undefined ){
						$element.css( 'background-size', styleJson[ 'background-size' ] );
					}
				}
				
				if( type == 'BASIC' ){
					if( styleJson.padding != undefined ){
						$element.css( 'color', styleJson.color  );
					}
				}
				if( type == 'BASIC' ){
					if( styleJson.border != undefined ){
						$element.css( 'border', styleJson.border  );
					}
				}
