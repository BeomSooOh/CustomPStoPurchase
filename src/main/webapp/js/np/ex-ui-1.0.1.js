...
(function ($) {
	var classes = {
		...
		rightHeaderDiv: 'cus_ta_ea ovh mr17 scbg ta_bl' /* header div class 정의 */ ,
		rightContentDiv: 'cus_ta_ea rowHeight scroll_y_fix rightContents scbg ta_bl' /* content div class 정의 */ ,
	...
	var methods = {
		...
		setRowData: function (parameters, focusX, focusY) {
			...
			/* log */
			if(id == 'tradeTable'){
				console.log('[setRowData] >> ' + JSON.stringify($.fn.extable.options['tradeTable'].data));
			}
		...
		getRowData: function (rowIndex) {
			...
			var id = $(this).prop('id');
			var main = $(this);
			// main.extable('setRecoveryArrayToJson');
		...
		getReqCheck: function (rowIdx) {
			...
			if (result.key.length > 0) {
				result.notInputCount = result.key.length;
				result.pass = false;
			} else {
				result.notInputCount = result.key.length;
				result.pass = true;
		...
		setRowDisable: function (positionX, positionY, disabled) {
			...
			for (var i = 0; i < $.fn.extable.options[id].colKey.length; i++) {
				if ($.fn.extable.options[id].columnRow[positionY][i] === undefined) {
					$.fn.extable.options[id].columnRow[positionY][i] = $.fn.extable.options[id].disabled[i].toString();
				}
			}
		...
		setValue: function (row, col, value) {
			...
			var main = $(this);
			/* 배열 데이터 정의 */
			$.fn.extable.options[id].arrayData[row][col] = value.toString();
			/* JSON 데이터 정의 */
			var key = $.fn.extable.options[id].column[0].colKey
			$.fn.extable.options[id].data[row][key] = value.toString();
			/* 그리드 표시정보 업데이트 */
			$(this).extable('setDisplayUpdate', row, col);
		},
		setDisplayUpdate: function (row, col) {
			var id = $(this).prop('id');
			var main = $(this);
			/* 표시 정보 조회 */
			var displayData = $.fn.extable.options[id].arrayData[row][col].toString();
			/* 테이블 정보 갱신 */
			var tableTr = $.fn.extable.options[id].rightContentTable.find('tr:eq(' + row + ')');
			if (tableTr.find('td:eq(' + col + ')').find('input').length > 0) {
				tableTr.find('td:eq(' + col + ')').find('input').val(displayData)
			} else {
				tableTr.find('td:eq(' + col + ')').html(displayData);
			}
		},
		setRecoveryArrayToJson: function(){
			var id = $(this).prop('id');
			var main = $(this);
			/* Array 정보 확인 */
			var Array = $.fn.extable.options[id].arrayData;
			var JsonData = [];
			/* JSON Data 생성 */
			for(var i=0; i<Array.length; i++){
				var JsonTemp = {};
				$.each($.fn.extable.options[id].colKey, function(idx, key){
					JsonTemp[key] = Array[i][idx];
				});
				
				JsonData.push(JsonTemp);
			}
			
			$.fn.extable.options[id].data = JSON.parse(JSON.stringify(JsonData));
		}
	};
	...
	var subMethods = {
		eventColCLick: function (targetCell) {
			...
			if ($.fn.extable.options[id].selectIdx[1] == targetCell.parent().index()) {
				if ($.fn.extable.options[id].selectIdx[0] == targetCell.index()) {
					if ($.fn.extable.options[id].rightContentTable.find('input').length) {
						return false;
					}
				}
			}
					...
					if (($.fn.extable.options[id].columnRow[afterY][afterX] || '') != 'disabled') {
						...
						$.each(comboData.list, function (itemIdx, item) {
							...
							$(li).click(function () {
								...
								// enter
								var len = $.fn.extable.options[id].rightContentTable.find('input').val().length
								if (($.fn.extable.options[id].rightContentTable.find('input')[0].selectionStart === len && $.fn.extable.options[id].rightContentTable.find('input')[0].selectionEnd === len) ||
									($.fn.extable.options[id].rightContentTable.find('input')[0].selectionStart === 0 && $.fn.extable.options[id].rightContentTable.find('input')[0].selectionEnd === len) ||
									($.fn.extable.options[id].rightContentTable.find('input')[0].selectionStart === 0 && $.fn.extable.options[id].rightContentTable.find('input')[0].selectionEnd === 0)) {
						...
						$.fn.extable.options[id].rightContentTable.find('tr:eq(' + ($.fn.extable.options[id].selectIdx[1]) + ') td:eq(' + (afterX + 2) + ')').click();
			...
			$(input).keydown(function (event) {
				...
				var beforeY = ($.fn.extable.options[id].oldSelectIdx[1] < 0 ? afterX : $.fn.extable.options[id].oldSelectIdx[1]);
				$.each($.fn.extable.options[id].arrayData[beforeY], function (idx, item) {
			...
			$(input).keyup(function (event) {
				...
				// keyUpParameters.beforeData = JSON.parse(JSON.stringify(main.extable('getRowData', afterY)));
			...
			$(input).dblclick(function (event) {
				if (typeof $.fn.extable.options[id].column[afterX].dblClickEvent === "function") {
					var clickParameters = {};
					clickParameters.data = main.extable('getRowData', beforeY);
					clickParameters.rowIndex = $.fn.extable.options[id].selectIdx[1];
					clickParameters.colIndex = $.fn.extable.options[id].selectIdx[0];
					clickParameters.id = id;
					clickParameters.tblId = id;
					clickParameters.keyCode = event.type;
					$.fn.extable.options[id].column[afterX].dblClickEvent(clickParameters || {});
				}
			});
			// input 조립
			$(span).append(input);
			...
			if (typeof $.fn.extable.options[id].clickAfter === 'function') {
				//if(beforeX >= 0 && $.fn.extable.options[id].column[beforeX].type !="combo") {
				$.fn.extable.options[id].clickAfter(afterParam);
				//}
			}

			/* tooltip 처리 */
			if (typeof $.fn.extable.options[id].tooltipFnc == 'function') {
				$.fn.extable.options[id].tooltipFnc();
			}
		...
		removeInp: function (focusX, focusY) {
			...
			//            $('#' + id + 'AutoComplete').contents().unbind().remove().hide();
		...
		setTabelRender: function () {
			...
			if ($.fn.extable.options[id].height) {
				$(rightContentDiv).css('height', $.fn.extable.options[id].height);
			}
		...
		getDefaults: function () {
			return {
				/* 자동 생성 */
				title: [],
				/* 그리드에서 column을 기초로 자동 생성합니다. 타이틀 정보를 배열로 가집니다. (function or string) */
				display: [],
				/* 그리드에서 column을 기초로 자동 생성합니다. 화면상 표현 여부 정보를 배열로 가집니다. (function or string) */
				displayKey: [],
				/* 그리드에서 column을 기초로 자동 생성합니다. 나타낼 문저열 조합의 정보를 배열로 가집니다. 구분자는 "▥" 를 사용합니다. (function or string) */
				colKey: [],
				/* 그리드에서 column을 기초로 자동 생성합니다. json 키 값 정보를 배열로 가집니다. */
				width: [],
				/* 그리드에서 column을 기초로 자동 생성합니다. 너비 정보를 배열로 가집니다. (function or string) */
				reqYN: [],
				/* 그리드에서 column을 기초로 자동 생성합니다. 필수 입력 정보를 배열로 가집니다. (function or string) */
				align: [],
				/* 그리드에서 column을 기초로 자동 생성합니다. 정렬 정보를 배열로 가집니다. (function or string) */
				type: [],
				disabled: [],
				arrayData: [],
				/* data와 동기화 됩니다. data는 json으로, arrayData는 배열로 정보를 저장합니다. */
				classes: [],
				/* 그리드에서 column을 기초로 자동 생성합니다. 사용자 정의 class 정보를 배열로 가집니다. (function or string) */
				mask: [],
				/* 그리드에서 column을 기초로 자동 생성합니다. mask 정보를 배열로 가집니다. (function or string) */
				rowSize: 0,
				/* y의 크기를 자동으로 저장합니다. */
				colSize: 0,
				/* x의 크기를 자동으로 저장합니다. */
				...
				leftHeaderTable: null,
				/* 왼쪽 헤더 테이블 */
				leftContentTable: null,
				/* 왼쪽 본문 테이블 */
				rightHeaderTable: null,
				/* 오른쪽 헤더 테이블 */
				rightContentTable: null,
				/* 오른쪽 본문 테이블 */
				/* 사용자 정의 */
				column: [],
				columnRow: [[]],
		...
		getDefaultsColumn: function () {
			return {
				...
				/* 정렬 */
				type: 'text',
				/* 입력 포맷 (text, combo, datepicker) */
				comboData: {},
				disabled: '',
				classes: '',
				mask: '',
				editor: {
					open: function (obj) { /* code pop or layer pop */ },
					close: function (obj) { /* code pop or layer pop callback */ }
				}
			}
		}
	};
	...
	$.fn.exSubTable = function (method) {
		if (subMethods[method]) {
			if ($.fn.extable && $.fn.extable.options && $.fn.extable.options['tradeTable']) {
				console.log('[' + method + '] >> ' + JSON.stringify($.fn.extable.options['tradeTable'].data));
			}
			return subMethods[method].apply(this, Array.prototype.slice.call(arguments, 1));
		} else {
			$.error('! 존재하지 않는 기능입니다...');
		}
	}
