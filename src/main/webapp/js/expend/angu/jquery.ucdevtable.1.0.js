    ...
    var p = {
        ...
        drawType2: function (pHeaderJSONList, pLocationId, pFirstAddRowYN, pTableType, pTableHeight, pBackgroundYN) {
            try {
                ...
                $(leftContentDiv).css('height', '100px');
        ...
        addrowCodeType1: function (pHeaderJsonList, pCodeJsonList, pLocationId) {
            ...
            for (var i = 0; i < pCodeJsonList.length; i++) {
                ...
                $(nTR).data('data', pCodeJsonList[i]);
                $(nTR).dblclick(function(){
                	var result = JSON.stringify($(this).data('data'));
                	var id = $('input.focus').prop('id').split('_')[0];
                	var continueFlag = true;

                	if(continueFlag){
	                	$.each(ABDOCU_0, function(idx, item){
	                		if(id == item.id){
	                			window[item.popupCustBindFunc]($('input.focus').prop('id'), JSON.parse(result));
	                		}
	                	});
                	}
                	if(continueFlag){
	                	$.each(ABDOCU_1, function(idx, item){
	                		if(id == item.id){
	                			window[item.popupCustBindFunc]($('input.focus').prop('id'), JSON.parse(result));
	                		}
	                	});
                	...
                	if(continueFlag){
	                	$.each(ABDOCU_B, function(idx, item){
	                		if(id == item.id){
	                			window[item.popupCustBindFunc]($('input.focus').prop('id'), JSON.parse(result));
	                		}
	                	});
                	...
                	if(continueFlag){
	                	$.each(ABDOCU_T, function(idx, item){
	                		if(id == item.id){
	                			window[item.popupCustBindFunc]($('input.focus').prop('id'), JSON.parse(result));
	                		}
	                	});
