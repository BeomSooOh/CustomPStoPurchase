/**
 * G20 연동 관련 js 파일 주로 ajax call 할때 사용한다.. 1. 최상위 객체 : acUtil 2. ajax handler : acUtil.ajax 3. 외부에서 ajax 호출하는 메서드 : acUtil.ajax.call(opt, extendObj) - opt : ajax 옵션 - extendObj : 결과 데이터 저장할 Object
 */
var acUtil = {};
...
acUtil.util.dialog.dialogDelegate = function ( fn, param, index, returnFn, tblParam ) {
	...
	if ( !acUtil.util.dialog.preProcessing ( param ) ) { return; }
	// edit mode 인지 체크(보류)
...
acUtil.util.dialog.setData = function ( paramMap, searchObj ) {
	...
	/**
	 * 거래처 정보 등 데이터가 많을 경우에는 grid 에 뿌려주지 않는다. 검색시에는 뿌려준다.
	 */
...
/*
 * acUtil.ajax.call_process 객체 인스턴스에 option(opt) 값 셋팅
 */
acUtil.ajax.set = function ( ajaxcall, opt ) {
	if ( !ajaxcall || !opt ) { return false; }
...
acUtil.util.getUniqueTime = function ( ) {
	...
	var timeUnique = "" + year + acUtil.util.setNumTwo ( month ) + acUtil.util.setNumTwo ( date ) + acUtil.util.setNumTwo ( hours ) + acUtil.util.setNumTwo ( minute ) + acUtil.util.setNumTwo ( sec ) + acUtil.util.setNumTwo ( millisec, true );
	return timeUnique;
};
...
acUtil.util.validMoneyKeyCode = function ( keycode ) {
	...
	if ( keycode == 8 || keycode == 13 || keycode == 9 | keycode == 16 || acUtil.util.isKeyCodeNum ( keycode ) || keycode == 189 || keycode == 109 || keycode == 190 || keycode == 110 ) {
		return true;
	} else {
		return false;
	}
...
function modal ( state ) {
	if ( state ) {
		...
		var img = $ ( "<img>" ).attr ( "id", "g20-ajax-img" ).attr ( "src", _g_contextPath_ + "/Images/ico/loading.gif" ).css ( "position", "absolute" ).css ( "top", "50%" ).css ( "left", "50%" ).css ( "margin-top", "-5" ).css ( "lefmargin-leftt", "-5" ).css ( "zIndex", "100000" );
		$ ( "body" ).append ( img );
	} else {
